...
## Git object cache — verification results (2026-06-03/04)

Feature: per-(org,repo) blobless bare mirror; each op `git clone --reference <mirror> --dissociate --filter=blob:none` so only deltas transfer over the network. Flag-gated `GIT_OBJECT_CACHE` (default off). Branch `feat/git-object-cache` (off `feat/performance-optimisations`). Service `server/ee/git-sync/git-object-cache.service.ts`; unit specs `server/test/modules/git-sync/unit/git-object-cache.service.spec.ts` (7 pass). Draft PRs: ToolJet#16678 / ee-server#576.

### Env
Local VPN-free. Backend :3230 (`NODE_ENV=production SERVE_CLIENT=false ENABLE_CORS=true node dist/src/main`), DB `tooljet_ee_feat_performance_optimisations` @ :5431, Redis :6379, OTEL→Tempo (service `tooljet-git-cache`). Repo `github.com/YuktiGoyal02/swiggy.git`. Org `tests-workspace` `1032c53c-…`, source branch `master` `d2f8564d-…` (591 DSVs), login `dev@tooljet.io`. Driven via authed `fetch` from the :3230 page, timed with `performance.now()`.

### A. Parity (correctness gate) — PASS
Create-branch from `master`, flag OFF (`gitcache-off-1`) vs ON (`gitcache-on-1`). DB content checksums + DSV count identical to source `master`:

| branch | app_meta_hash | data_source_meta_hash | module_meta_hash | DSVs |
|---|---|---|---|---|
| master | `29e5d784…523f` | `a2f3a9bc…1b2f` | `4f10be56…6d9a` | 591 |
| gitcache-off-1 | `29e5d784…523f` | `a2f3a9bc…1b2f` | `4f10be56…6d9a` | 591 |
| gitcache-on-1 | `29e5d784…523f` | `a2f3a9bc…1b2f` | `4f10be56…6d9a` | 591 |

ON output byte-identical to OFF. (Full hashes match; truncated for the table.)

### B. Performance (wall-clock, local VPN-free)
| flow | OFF (fresh clone) | ON cold (seeds mirror) | ON warm (delta) |
|---|---|---|---|
| check-updates (`master`, paths=[]) | 2478–2993 ms | 3868 ms | 2272–2559 ms |
| create-branch (from `master`) | 9762 ms | — | 7434 ms (~24% faster, ~2.3 s git-phase saved) |

Local win is modest by design: VPN-free the full clone is already cheap, so the fetch-before-use network round-trip + local reference-clone dominate. The large win (design: 57 s→<1 s) is a high-latency/VPN effect where object transfer dominates — not reproducible locally. create-branch's remaining ~7.4 s is the unchanged DB work (591 DSV inserts + folder_apps), not the clone.

Note on git-phase tracing: the git clone runs as a child process (simple-git), not OTEL-instrumented, so it shows only as a gap between DB spans in Tempo — wall-clock is the cleaner metric here.

### C. Cache engaged — PASS
Mirror created at `<tmpdir>/tj-git-cache/0ab98eaf…523…ab.git`, **7.2 MB blobless** (commit/tree only, no file blobs). Server log: **zero `[git-cache]` fallback warnings** across all ON runs → the cache path (not plainClone) served every request.

### D. Security / token hygiene — PASS
Mirror `config` `remote.origin.url = https://github.com/YuktiGoyal02/swiggy.git` (clean, `promisor = true`). Recursive grep of the entire mirror (`config`, `packed-refs`, `logs/`, `FETCH_HEAD`, all files) for `x-access-token` / `ghs_` / `github_pat_` / `://user:pass@` → **none**. Token rides only in per-call `-c http.extraHeader=Authorization: Basic …`, never persisted.

### E. Resilience
- **Self-heal:** corrupted the mirror's pack (`printf CORRUPTED > pack-*.pack`, confirmed via `git fsck` "not a GIT packfile"). The cache path on the next op catches the broken-alternates clone error → `selfHeal` rm's the mirror → `plainClone` fallback → mirror rebuilt next op. (Mechanism verified at code + unit level; live end-to-end firing was cut short — mirror was removed to complete the heal.)
- **Eviction (TTL / size-cap / specific evict):** covered by unit specs (Task 7) — 7/7 pass. Live disconnect-evict (Redis broadcast) not exercised against the live org (destructive to its git config); wiring verified: `deleteConfig` → `evictEverywhere` → local evict + publish `tj:git-cache:evict`; subscriber wired in `onModuleInit` (flag-on boot confirmed clean, "Nest application successfully started", no DI error).

### Boot / DI
Flag-ON boot succeeded with the single consolidated `GitObjectCacheService` instance (owned + exported by `GitSyncModule`; `WorkspaceBranchesModule` injects via existing import). No "Nest can't resolve" errors.

### Cleanup pending
Test branches created against the live repo: `gitcache-off-1`, `gitcache-on-1`, `gitcache-on-diag` (+ remote pushes for off-1/on-1). Delete via the branch-delete endpoint or UI.
