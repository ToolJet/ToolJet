...
:::info
- It is mandatory to provide a `id` for each column in the `column data` field. The `id` can be of type `string` or `number`.
- It is mandatory to provide a `id` and a `columnId` for each card in the `Card data` field. Both `id` and `columnId` can be of type `string` or `number`.
:::
...
| Enable card deletion zone | This property allows you to show or hide the **Drop here to delete** section at the bottom of the Kanban. | By default, its enabled, you can programmatically set value to `{{true}}` or `{{false}}` to enable/disable button by clicking on the **fx** next to it. |
| Delete zone label | Change the label for the delete zone at the bottom of the kanban board. | Default value - `Drop here to delete` |
| Open modal on card click | Opens a modal when a card is selected. | By default, its enabled, you can programmatically set value to `{{true}}` or `{{false}}` to enable/disable button by clicking on the **fx** next to it. |
| Modal size | Select the modal size. | Choose from - small, medium, large or full-screen |
| Height | Enter the modal height. | Enter the value in pixels. |
...
| <div style={{ width:"100px"}}> Variable </div> | <div style={{ width:"300px"}}> Description </div> | <div style={{ width:"135px"}}> How To Access </div> |
| :--------------------------------------------- | :------------------------------------------------ | :-------------------------------------------------- |
...
| lastCardUpdate | The variable `lastCardUpdate` holds the old and new values of the property that has been changed in the card(using component specific action). | Get the values using `{{components.kanban1.lastCardUpdate[0].title.oldValue}}`. |
...
| :-------------------------------------------- | :------------------------------------------------ | :-------------------------------------------------- |
| updateCardData | Update the card data of Kanban component via a component-specific action within any event handler. | `components.kanban1.updateCardData('c1', {title: 'New Title'})` |
| moveCard | Move a card from one column to other column on the kanban via a component-specific action within any event handler. | `components.kanban1.moveCard('card id,'column id')` |
| addCard | Add a card onto the kanban via a component-specific action within any event handler. | `components.kanban1.addCard('c1', {title: 'New Title'})` |
| deleteCard | Delete a card from the kanban via a component-specific action within any event handler. | `components.kanban1.deleteCard('card id')` |

## Additional Actions

| <div style={{ width:"100px"}}> Action </div> | <div style={{ width:"150px"}}> Description </div> |
| :------------------------------------------- | :------------------------------------------------ |
| Tooltip | Provides additional information on hover. Set a display string. |
## Devices
