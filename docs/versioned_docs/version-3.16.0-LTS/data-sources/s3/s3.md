...
:::info
ToolJet supports overriding AWS S3 connection parameters at **query runtime** when dynamic connection parameters are enabled for the datasource.

Once enabled, the **bucket parameter** becomes configurable directly within the query builder, allowing bucket values to be supplied dynamically at execution time instead of being statically defined in the datasource configuration. This eliminates the need to create separate datasource configurations for each bucket when the connection credentials remain the same, making queries more reusable and easier to maintain.
:::
...
:::tip
You can now connect to **[different S3 Hosts using custom endpoints](/docs/data-sources/s3/s3-custom-endpoints)**.
:::
