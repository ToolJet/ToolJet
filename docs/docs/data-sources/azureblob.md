...
To connect ToolJet with the Azure Blob data sources, you have two options:
1. Click on the `+Add new data sources` button in the query panel.
2. Go to the **[data sourcess](/docs/data-sources/overview)** page on the ToolJet dashboard.
...
Once you have connected to the Azure Blob data sources, follow these steps to create queries and interact with a Azure Blob storage from the ToolJet application:

1. Open the ToolJet application and navigate to the query panel at the bottom of the app builder.
2. Click the `+Add` button to open the list of available `local` and `data sourcess`.
3. Select **Azure Blob** from the data sources section.
