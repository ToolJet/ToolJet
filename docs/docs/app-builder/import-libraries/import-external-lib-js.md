...
The **Libraries** section on the left property panel lets you register external libraries once at the app level. Registered libraries are loaded at app startup and available by their variable name in all RunJS queries and transformations — no per-query import code needed.
