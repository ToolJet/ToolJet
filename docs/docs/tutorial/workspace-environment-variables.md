...
- **Server**: The server variables can be used with all the queries except the `RunJS` or `RunPy`. The reason why we don't allow the server variables to be used with the componenets is that these variables are only resolved during the runtime so they're highly secured.
