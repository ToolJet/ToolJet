---
id: kubernetes-gke
title: Déployer ToolJet sur Kubernetes (GKE)
slug: /setup/kubernetes-gke/
...
Suivez les étapes ci-dessous pour déployer ToolJet sur un cluster Kubernetes GKE.

1. Créez un certificat SSL.
   ```bash
   curl -LO https://tooljet-deployments.s3.us-west-1.amazonaws.com/kubernetes/GKE/certificate.yaml
   ```
   Remplacez le nom de domaine par le domaine/sous-domaine que vous souhaitez utiliser pour l'installation de ToolJet.
2. Réservez une adresse IP statique avec la CLI `gcloud`
   ```bash
   gcloud compute addresses create tj-static-ip --global
   ```
3. Créez le déploiement k8s
   ...
   **Flexibilité de déploiement :**
   ...
   :::info
   Pour des variables d'environnement supplémentaires, consultez notre [documentation sur les variables d'environnement](/docs/setup/env-vars). S'il existe des points de terminaison HTTPS auto-signés auxquels ToolJet doit se connecter, assurez-vous que la variable d'environnement `NODE_EXTRA_CA_CERTS` est définie sur le chemin absolu contenant les certificats. Vous pouvez utiliser les secrets Kubernetes pour monter le fichier de certificat sur les conteneurs.
   :::

4. Créez le service k8s
   ```bash
   curl -LO https://tooljet-deployments.s3.us-west-1.amazonaws.com/kubernetes/GKE/service.yaml
   ```
5. Créez l'ingress k8s
   ```bash
   curl -LO https://tooljet-deployments.s3.us-west-1.amazonaws.com/kubernetes/GKE/ingress.yaml
   ```
   Remplacez le nom de domaine par le domaine/sous-domaine que vous souhaitez utiliser pour l'installation de ToolJet.
6. Appliquez les configurations YAML
   ```bash
   kubectl apply -f certificate.yaml, deployment.yaml, service.yaml, ingress.yaml
   ```

:::info
Le provisionnement des certificats managés peut prendre quelques minutes. [Documentation sur les certificats managés](https://cloud.google.com/kubernetes-engine/docs/how-to/managed-certs).
:::

Vous pourrez accéder à votre installation ToolJet une fois que les pods, le service et l'ingress seront en cours d'exécution.
