---
id: kubernetes
title: Déployer ToolJet sur Kubernetes
...
Suivez les étapes ci-dessous pour déployer ToolJet sur un cluster Kubernetes.

1. **Configurez les bases de données PostgreSQL**

   ToolJet nécessite **deux bases de données PostgreSQL distinctes**. Nous n'avons pas l'intention de prendre en charge d'autres bases de données telles que MySQL.

2. **Créez un secret Kubernetes nommé `server`**

   Configurez toutes les variables d'environnement requises pour le déploiement ToolJet :
...
3. Créez un déploiement Kubernetes
   ...
   :::info
   Le fichier ci-dessus n'est qu'un modèle et pourrait ne pas convenir aux environnements de production. Vous devriez télécharger le fichier et configurer des paramètres tels que le nombre de réplicas et les variables d'environnement selon vos besoins.

   S'il existe des points de terminaison HTTPS auto-signés auxquels ToolJet doit se connecter, veuillez vous assurer que la variable d'environnement `NODE_EXTRA_CA_CERTS` est définie sur le chemin absolu contenant les certificats. Vous pouvez utiliser les secrets Kubernetes pour monter le fichier de certificat sur les conteneurs.
   :::

4. Vérifiez si ToolJet est en cours d'exécution
   ```bash
   kubectl get pods
   ```
5. Créez des services Kubernetes pour publier le déploiement Kubernetes que vous avez créé. Cette étape varie selon les fournisseurs cloud. Nous avons un [modèle](https://tooljet-deployments.s3.us-west-1.amazonaws.com/kubernetes/service.yaml) pour exposer le serveur ToolJet en tant que service via un load balancer AWS. <br/>
   **Exemples :**
   - [Répartition de charge d'application sur Amazon EKS](https://docs.aws.amazon.com/eks/latest/userguide/alb-ingress.html)
   - [Ingress GKE pour la répartition de charge HTTP(S)](https://cloud.google.com/kubernetes-engine/docs/concepts/ingress)
