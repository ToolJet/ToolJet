---
id: openshift
title: Déployer ToolJet sur Openshift
slug: /setup/openshift/
...
Suivez les étapes ci-dessous pour déployer ToolJet sur Openshift.

### 1. Configurer les variables d'environnement requises

ToolJet nécessite **deux bases de données PostgreSQL distinctes** et plusieurs variables d'environnement pour la configuration initiale. Configurez-les toutes avant le déploiement :
...
2. Une fois connecté au tableau de bord développeur Openshift, cliquez sur l'onglet `+Add`. Sélectionnez importer un YAML depuis la machine locale.
   :::note
   Lors de la saisie d'un ou plusieurs fichiers, utilisez --- pour séparer chaque définition
   :::
   Copiez-collez deployment.yaml dans l'éditeur en ligne

   ```
   curl -LO https://tooljet-deployments.s3.us-west-1.amazonaws.com/openshift/deployment.yaml
   ```

   Copiez-collez service.yaml dans l'éditeur en ligne
   ...
   Une fois les fichiers ajoutés, cliquez sur créer.

3. Accédez à l'onglet topology et utilisez le connecteur visuel pour établir la connexion entre tooljet-deployment et postgresql comme illustré dans la capture d'écran ci-dessous.
