...
**`app-auth.guard.ts`** — current logic:
```typescript
// Current: queries apps table directly
const app = await this.appRepository.findOne(App, { where: { slug } });
if (app.isPublic === true) { /* allow */ }
```

New logic:
```typescript
// New: resolve through current_version_id for released routing
const app = await this.appRepository
  .createQueryBuilder('app')
  .innerJoin('app_versions', 'av', 'av.id = app.current_version_id')
  .where('av.slug = :slug', { slug })
  .andWhere('app.organization_id = :orgId', { orgId })
  .getOne();

// Check is_public from the released version
const releasedVersion = await this.versionRepository.findOne({ where: { id: app.currentVersionId } });
if (releasedVersion.is_public === true) { /* allow */ }
```

**`valid-slug.guard.ts`** — already reads `branch_id` from request. Update `findAppWithIdOrSlug` to:
- If `branchId` provided: resolve via `app_versions WHERE slug = :slug AND branch_id = :branchId`
- If no `branchId`: resolve via `app_versions WHERE slug = :slug AND branch_id = defaultBranchId`
- If not found → throw `NotFoundException` (no fallback)

**`public_app_environment.guard.ts`** — same pattern as `app-auth.guard.ts`, resolve through released version.
...
| Method | Current | New |
|--------|---------|-----|
| `findBySlug(slug, orgId, branchId)` | `apps WHERE slug = :slug` | `app_versions av JOIN apps a ON av.app_id = a.id WHERE av.slug = :slug AND a.organization_id = :orgId AND av.branch_id = :branchId` |
| `retrieveAppDataUsingSlug(slug)` | Returns `orgId, isPublic, isReleased` from `apps` | JOINs through `current_version_id` for released context; uses `branch_id` for editor context |
| `findAllOrganizationApps(orgId, branchId)` | `SELECT app.slug, app.name` | `JOIN app_versions ON branch_id = :activeBranchId AND versionType = 'BRANCH'`, selects `av.slug, av.app_name` |
...
**`afterInsert`:**
- Current: sets `apps.slug = app.id` if slug not provided
- New: for apps/modules, also create initial `app_versions` row with `slug = app.id` (if not already created by the service layer)
- For workflows: unchanged

**`afterLoad`:**
- Current: loads `editingVersion` filtering out BRANCH-type versions
- New: for apps/modules, attach `slug`, `app_name`, `icon`, `is_public` from the loaded version to the entity (for backward compatibility with code that reads `app.slug`)
- For workflows: unchanged (reads from `apps.*`)
