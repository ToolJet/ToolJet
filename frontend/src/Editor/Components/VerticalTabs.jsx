...
export const VerticalTabs = function VerticalTabs({
    id,
    component,
    width,
    height,
    ...
    const tabNavWidth = component.definition.properties?.tabNavWidth?.value ?? 50;
    ...
    const parsedtabNavWidth = typeof tabNavWidth !== 'number' ? tabNavWidth == "{{}}" ? 0 : resolveWidgetFieldValue(tabNavWidth, currentState) : tabNavWidth;
        ...
        >
            ...
            >
                {parsedTabs.map((tab) => (
                    <li
                        className="nav-item"
                        style={{ opacity: tab?.disabled && '0.5'}}
                        onClick={() => {
