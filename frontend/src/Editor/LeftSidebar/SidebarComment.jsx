...
export const LeftSidebarComment = forwardRef(
  (
    { selectedSidebarItem, currentPageId, isVersionReleased, isEditorFreezed },
    ref
  ) => {
    ...
    const shouldEnableComments =
      window.public_config?.ENABLE_COMMENTS === "true";
    ...
    React.useEffect(() => {
      async function fetchData() {
        try {
          ...
          setBasicPlan(
            data?.licenseStatus?.isExpired ||
              !data?.licenseStatus?.isLicenseValid
          );
        } catch (error) {
    ...
    React.useEffect(() => {
      if (isActive) {
        commentsService
          .getNotifications(appId, false, appVersionsId, currentPageId)
          .then(({ data }) => {
    ...
    return basicPlan ? (
      ...
      <LeftSidebarItem
        ...
        icon={"comments"}
        className={cx(
          `left-sidebar-item sidebar-comments left-sidebar-layout sidebar-comments`,
          {
            disabled:
              !appVersionsId ||
              isVersionReleased ||
