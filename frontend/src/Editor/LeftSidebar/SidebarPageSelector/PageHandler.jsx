...
export const AddingPageHandler = ({ addNewPage, setNewPageBeingCreated, darkMode }) => {
  ...
  const handleAddingNewPage = (pageName) => {
    if (pageName.trim().length === 0) {
      toast(t('leftSidebar.pageHandler.nameMinChar', 'Page name should have at least 1 character'), {
        icon: '⚠️',
      });
    } else if (pageName.trim().length > 50) {
      toast(t('leftSidebar.pageHandler.nameMaxChar', 'Page name cannot exceed 50 characters'), {
        icon: '⚠️',
      });
    } else {
