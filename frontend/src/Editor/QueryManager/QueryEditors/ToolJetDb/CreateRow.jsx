...
export const CreateRow = ({ currentState, optionchanged, options, darkMode }) => {
  ...
  const [createColumnOptions, setCreateColumnOptions] = useState(options['create_row'] || { 0: {} });
  ...
  useEffect(() => {
    optionchanged('create_row', createColumnOptions);
  }, [createColumnOptions]);
  ...
  const handleColumnChange = (index, value) => {
    const prevCreateColumnOptions = { ...createColumnOptions };
    prevCreateColumnOptions[index].column = value;

    setCreateColumnOptions(prevCreateColumnOptions);
  };

  const handleValueChange = (index, value) => {
    const prevCreateColumnOptions = { ...createColumnOptions };
    prevCreateColumnOptions[index].value = value;
  ...
  const handleDelete = (index) => {
    const prevCreateColumnOptions = { ...createColumnOptions };
    delete prevCreateColumnOptions[index];
    setCreateColumnOptions(prevCreateColumnOptions);
  };
  ...
  return (
    <>
      ...
      {createColumnOptions[0] &&
        Object.values(createColumnOptions).map((columnOption, index) => {
          return (
