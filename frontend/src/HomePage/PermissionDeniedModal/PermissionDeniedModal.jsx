...
export const PermissionDeniedModal = ({ onHide, ...props }) => {
  ...
  const createWorkspace = () => {
    ...
    const workspaceName = `${t('permissionDeniedModal.defaultWorkspaceName', 'My Workspace')}${timestamp}`;
    ...
    organizationService.createOrganization({ name: workspaceName, slug: workspaceSlug }).then(
      (data) => {
        ...
        toast.success(t('permissionDeniedModal.workspaceCreated', 'Workspace created successfully'));
        setIsCreating(false);
      ...
      (error) => {
        ...
        if (error?.error && error.error.includes('reached your limit for number of builders')) {
          toast.error(
            t(
              'permissionDeniedModal.builderLimitExceeded',
      ...
      >
        <Modal.Header closeButton onHide={onHide} style={{ padding: '15px' }}>
          <span className="header-text" data-cy="modal-header" style={{ marginTop: '10px', lineHeight: '24px' }}>
            {t('permissionDeniedModal.title', 'Permission Denied')}
          </span>
          <p
          ...
          >
            {t(
              'permissionDeniedModal.description',
              "You don't have permission to create apps in this workspace. Would you like to create the app in a new workspace?"
            )}
          </p>
        </Modal.Header>
        <Modal.Footer>
          ...
          <ButtonSolid onClick={createWorkspace}>
            {t('permissionDeniedModal.createInNewWorkspace', 'Create app in a new workspace')}
          </ButtonSolid>
        </Modal.Footer>
      </Modal>
    </div>
