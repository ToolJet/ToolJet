...
const Table = () => {
  ...
  const handleDelete = async () => {
    ...
    // });
    // const { error } = await tooljetDatabaseService.delete(organizationId, selectedTable, selectedRows);
  ...
  return (
    <div>
      {Object.keys(selectedRowIds).length > 0 && (
        <button onClick={handleDelete} type="button" className="btn no-border d-flex align-items-center mb-3">
            ...
            {headerGroups.map((headerGroup, index) => (
              <tr {...headerGroup.getHeaderGroupProps()} key={index}>
                {headerGroup.headers.map((column, index) => (
                  <TablePopover key={index} disabled={index === 0}>
