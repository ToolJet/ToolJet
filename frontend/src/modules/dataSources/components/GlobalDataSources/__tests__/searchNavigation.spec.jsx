...
jest.mock('../../../pages/GlobalDataSourcesPage', () => ({ GlobalDataSourcesContext: require('react').createContext({}) }));
...
jest.mock('@/modules/common/components/DataSourceComponents', () => ({
  DataBaseSources: [], ApiSources: [], CloudStorageSources: [], AiSources: [], getCommonlyUsedDataSources: () => [],
}));
jest.mock('@/_services', () => ({
  pluginsService: { findAll: jest.fn() }, marketplaceService: { findAll: jest.fn() },
  globalDatasourceService: { create: jest.fn() }, authenticationService: { currentSessionValue: {} },
}));
jest.mock('@/_ui/Card', () => ({ Card: ({ title }) => <div>{title}</div> }));
jest.mock('@/MarketplacePage/MarketplaceCard', () => ({ MarketplaceCard: ({ name }) => <div>{name}</div> }));
jest.mock('@/_components', () => ({ SearchBox: ({ initialValue, callBack }) => <input aria-label="Search" value={initialValue} onChange={callBack} /> }));
...
function mount(child) {
  return render(<MemoryRouter initialEntries={['/?search=Ledger']}>
...
const sources = [{ id: 'ledger', kind: 'ledger', name: 'Ledger', tags: [] }, { id: 'queue', kind: 'queue', name: 'Queue', tags: [] }];
...
test('URL search includes asynchronously loaded plugins and follows navigation', async () => {
  let loaded;
  pluginsService.findAll.mockReturnValue(new Promise((resolve) => { loaded = resolve; }));
