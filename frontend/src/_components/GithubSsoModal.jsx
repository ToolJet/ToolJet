...
export function GithubSSOModal({
  settings,
  onClose,
  onUpdateSSOSettings,
  ...
  const saveInstanceSettings = () => {
    ...
    instanceSettingsService
      .updateSSOConfigs({ type: 'git', configs: { clientId, clientSecret, hostName }, enabled: enabled })
      .then(
        (data) => {
          setSaving(false);
          data.id && setConfigId(data.id);
  ...
  const initiateSave = () => {
    if (!instanceLevel && enabled != settings?.enabled && enabled === true && isInstanceOptionEnabled('git')) {
