...
import { sessionService, appService, authenticationService } from '@/_services';
import { CopyToClipboard } from 'react-copy-to-clipboard';
import { CircleAlert, Copy, CircleCheckBig } from 'lucide-react';
  ...
  useEffect(() => {
    sessionService
      ...
      .then(({ current_organization_id, ...restSessionData }) => {
        // Populate the org context before calling any org-scoped endpoint below (e.g.
        // getRestrictedAccessInfo) — otherwise authHeader() has no tj-workspace-id to send yet
        // on a fresh page load, and the request 401s even for a fully valid session.
        authenticationService.updateCurrentSession({ current_organization_id, ...restSessionData });
...
// Plain-text equivalent of the app-details list items (bullets there are CSS list markers, not literal characters).
const buildAppDetailsText = (details, appUrl) => {
  const lines = [`App: ${details.appName}`];
  if (details.folderName) {
    lines.push(`Folder: ${details.folderName}`);
  }
...
export const ErrorModal = ({ errorMsg, appSlug, isValidSession, errorType, appDetails, ...props }) => {
  ...
  const isRestrictedType = errorType === ERROR_TYPES.RESTRICTED;
  const showAppDetails = isRestrictedType && isValidSession && !!appDetails;
  const subpath = getSubpath() ?? '';
  const appUrl = `${window.location.origin}${subpath}/applications/${appSlug}`;
  const description = showAppDetails
  ...
  return (
    <div className="custom-backdrop">
      ...
      >
        <Modal.Header>
          {isRestrictedType ? (
            <CircleAlert className="restricted-access-icon" size={40} />
          ) : (
          ...
          {isRestrictedType ? (
            <div className="restricted-access-message">
              ...
              <p className="description" data-cy="modal-description">
                {t('globals.static-error-modal.description', description)}
              </p>
            </div>
          ) : (
            <>
          ...
          {showAppDetails && (
            <div className="restricted-access-details" data-cy="restricted-access-details">
              <ul>
                <li>App: {appDetails.appName}</li>
                {appDetails.folderName && <li>Folder: {appDetails.folderName}</li>}
                <li>URL: {appUrl}</li>
              </ul>
              <svg
                className="restricted-access-grip"
                width="7"
                height="7"
                viewBox="0 0 6.70711 6.70711"
                fill="none"
                aria-hidden="true"
              >
                <path d="M3.95355 6.35355L6.35355 3.95355M6.35355 0.353553L0.353553 6.35355" stroke="#ACB2B9" />
              </svg>
            </div>
          )}
        </Modal.Header>
        <Modal.Footer>
          ...
          {showAppDetails && (
            <CopyToClipboard text={buildAppDetailsText(appDetails, appUrl)} onCopy={() => setCopied(true)}>
              <button className={`btn restricted-copy-btn${copied ? ' copied' : ''}`} data-cy="copy-app-details-button">
                {copied ? <CircleCheckBig size={16} /> : <Copy size={16} />}
                {copied ? 'Copied!' : t('globals.workspace-modal.continue-btn', 'Copy app details')}
              </button>
            </CopyToClipboard>
          )}
        </Modal.Footer>
