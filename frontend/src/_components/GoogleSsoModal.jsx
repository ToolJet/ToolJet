...
export function GoogleSSOModal({
  ...
  const saveInstanceSettings = () => {
    setSaving(true);
    instanceSettingsService.updateSSOConfigs({ type: 'google', configs: { clientId }, enabled: enabled }).then(
      (data) => {
        setSaving(false);
        data.id && setConfigId(data.id);
        onUpdateSSOSettings('google', { id: data?.id || configId, configs: { client_id: clientId }, enabled: enabled });
  ...
  const initiateSave = () => {
    if (!instanceLevel && enabled != settings?.enabled && enabled === true && isInstanceOptionEnabled('google')) {
