...
export function ImportApp({ closeModal, importApp, fileContent, show }) {
  const selectedAppName = fileContent?.appV2?.name || '';
  ...
  const handleImportApp = async (fileContent) => {
    if (!errorText) {
      setIsLoading(true);
      try {
        const success = await importApp(fileContent, newAppName);
        if (success === false) {
