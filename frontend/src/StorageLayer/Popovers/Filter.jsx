...
const Form = ({ setFilters, index }) => {
  ...
  const handleFilterChange = (selectedOption) => {
    setFilters((prevFilters) => {
      prevFilters[index].filter = selectedOption.value;
      return prevFilters;
    });
  };
  ...
  return (
    <div className="row g-2 align-items-center">
      <div className="col-3 py-3">
        <Select options={columns} onChange={handleColumnChange} />
      </div>
      <div className="col-3 py-3">
        <Select options={filters} onChange={handleFilterChange} />
      </div>
      <div className="col-3 py-3">
        <input type="text" className="form-control" placeholder="Value" onChange={handleInputChange} />
      </div>
...
const Filter = ({ query }) => {
  ...
  const handleBuildQuery = () => {
    ...
    keys.map((key) => {
      const { column, filter, value } = filters[key];
      if (keys.length === 1) {
        ...
        // TODO: add more cases;
        postgrestFilterBuilder[filter](column, value);
      }
    });
  ...
  return (
    <>
      <button ref={ref} onClick={handleClick} className="btn btn-outline">
          ...
          <Popover.Content bsPrefix="storage-filter-popover">
            <div className="card-body">
              {Object.keys(filters).map((filter, index) => {
                return <Form key={index} index={index} setFilters={setFilters} />;
              })}
            </div>
            <div
