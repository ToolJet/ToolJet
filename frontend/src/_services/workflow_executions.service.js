...
function create(appVersionId, testJson) {
  const currentSession = authenticationService.currentSessionValue;
  const body = { appVersionId, userId: currentSession.current_user?.id, executeUsing: 'version', params: testJson };
...
function execute(workflowAppId, params, appId = undefined, appEnvId) {
  const currentSession = authenticationService.currentSessionValue;
  const body = {
    appId: workflowAppId,
    app: appId,
