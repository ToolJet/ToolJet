...
// transient bookkeeping, the same pattern resolvedSlice.js's batch manager uses, and (like
// that manager) lives inside the slice factory so it's recreated fresh per store instance.
...
export const createEventsSlice = (set, get) => {
  const eventCallbackQueue = createEventCallbackQueue();
  ...
  return {
    ...
    eventsSlice: {
      ...
      fireEvent: (eventName, id, moduleId, customResolvables, options, sourceSubContainerIndex = null) => {
        ...
        eventCallbackQueue.drain(moduleId, id, eventName, () => get().eventsSlice.getLiveComponents(moduleId));
      },
      ...
      queueEventCallback: (sourceId, eventName, callback, moduleId = 'canvas') =>
        eventCallbackQueue.push(moduleId, sourceId, eventName, callback),
      ...
      updateAppVersionEventHandlers: async (events, updateType = 'update', param, moduleId = 'canvas') => {
        ...
        const appId = get().appStore.modules[moduleId].app.appId;
        const versionId = get().currentVersionId;
        const newEvents = replaceQueryOptionsEntityReferencesWithIds(
          events,
