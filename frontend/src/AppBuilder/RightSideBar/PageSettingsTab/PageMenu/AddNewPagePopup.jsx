...
const OPEN_APP_MODES = ['new_tab', 'same_tab'];
...
export const AddEditPagePopup = forwardRef(({ darkMode, ...props }, ref) => {
  ...
  const pagePrefix = t('editor.pageMenu.defaultNames.page', 'Page');
  const urlPrefix = t('editor.pageMenu.defaultNames.url', 'URL');
  const appPrefix = t('editor.pageMenu.defaultNames.app', 'App');
  const groupPrefix = t('editor.pageMenu.defaultNames.group', 'Group');
  ...
  useEffect(() => {
    if (mode === 'add' && type === 'default' && !hasAutoSaved) {
      ...
      let index = 1;
      let newName = `${pagePrefix} ${index}`;
      while (existingNames.includes(newName.toLowerCase())) {
        index++;
        newName = `${pagePrefix} ${index}`;
      }
  ...
  }, [mode, hasAutoSaved, pages, editingPage, addNewPage, isPageGroup, type, pagePrefix]);
  ...
  useEffect(() => {
    if (mode === 'add' && type === 'url' && !hasAutoSaved) {
      const existingNames = pages.map((p) => p.name.toLowerCase());
      let index = 1;
      let newName = `${urlPrefix} ${index}`;
      while (existingNames.includes(newName.toLowerCase())) {
        index++;
        newName = `${urlPrefix} ${index}`;
      }
  ...
  }, [addNewPage, appOptions, editingPage, hasAutoSaved, isPageGroup, mode, pages, type, urlPrefix]);
  ...
  useEffect(() => {
    ...
    if (mode === 'add' && type === 'app' && !hasAutoSaved) {
      const existingNames = pages.map((p) => p.name.toLowerCase());
      let index = 1;
      let newName = `${appPrefix} ${index}`;
      while (existingNames.includes(newName.toLowerCase())) {
        index++;
        newName = `${appPrefix} ${index}`;
      }
  ...
  }, [mode, hasAutoSaved, pages, editingPage, addNewPage, isPageGroup, type, appId, appPrefix]);
  ...
  useEffect(() => {
    if (mode === 'add' && type === 'group' && !hasAutoSaved) {
      const existingNames = pages.map((p) => p.name.toLowerCase());
      let index = 1;
      let newName = `${groupPrefix} ${index}`;
      while (existingNames.includes(newName.toLowerCase())) {
        index++;
        newName = `${groupPrefix} ${index}`;
      }
  ...
  }, [mode, hasAutoSaved, pages, editingPage, addNewPage, isPageGroup, type, appId, groupPrefix]);
    ...
    >
      <Popover.Header>
        <div className="d-flex justify-content-between align-items-center">
          <div className="tj-text-xsm font-weight-500 text-default">
            {t(`editor.pageMenu.popoverTitles.${mode}.${type}`, POPOVER_TITLES?.[mode]?.[type])}
          </div>
          <div className="actions-container">
            ...
            <ToolTip
              message={t('editor.pageMenu.tooltips.duplicateEntity', {
                defaultValue: 'Duplicate {{entity}}',
                entity: t(type === 'group' ? 'editor.pageMenu.groupLower' : 'globals.page', type === 'group' ? 'group' : 'page'),
              })}
              placement="bottom"
            >
            ...
            <ToolTip
              message={t('editor.pageMenu.tooltips.deleteEntity', {
      ...
      <Popover.Body className={`${darkMode && 'dark-theme'}`}>
        {type === 'default' && (
          <>
            <div className="pb-2">
              <div className="col">
                <label className="form-label font-weight-400 mb-0">{t('editor.pageMenu.labels.pageName', 'Page name')}</label>
                <input
                  type="text"
            ...
            <div className="pb-2">
              <div className="col">
                <label className="form-label font-weight-400 mb-0">{t('editor.pageMenu.labels.handle', 'Handle')}</label>
                <input
                  type="text"
            ...
            <div className="pb-1">
              <div className="d-flex justify-content-between align-items-center pb-2">
                <label className="form-label font-weight-400 mb-0">{t('editor.pageMenu.labels.icon', 'Icon')}</label>
                <Icon
            ...
            <div className="pb-2">
              <div className=" d-flex justify-content-between align-items-center pb-2">
                <label className="form-label font-weight-400 mb-0">
                  {t('editor.pageMenu.labels.markAsHome', 'Mark as home')}
                </label>
              ...
              <div className=" d-flex justify-content-between align-items-center pb-2">
                <label className="form-label font-weight-400 mb-0">
                  {t('editor.pageMenu.labels.disablePage', 'Disable page')}
                </label>
                <label className={`form-switch`}>
                  <input
                    className="form-check-input"
        ...
        {type === 'url' && (
          <>
            <div className="pb-2">
              <div className="col">
                <label className="form-label font-weight-400 mb-0">{t('globals.label', 'Label')}</label>
                <input
                  type="text"
                  onChange={(e) => setPageName(e.target.value)}
                  className="form-control"
            ...
            <div className="pb-2">
              <div className="col">
                <label className="form-label font-weight-400 mb-0">{t('editor.pageMenu.labels.url', 'URL')}</label>
                <textarea
            ...
            <div className="d-flex justify-content-between align-items-center pb-2">
              <label className="form-label font-weight-400 mb-0">
                {t('editor.pageMenu.labels.openUrlIn', 'Open URL in')}
              </label>
                ...
                >
                  {OPEN_APP_MODES.map((mode) => (
                    <ToggleGroupItem key={mode} value={mode}>
                      {t(
                        `editor.pageMenu.openModes.${mode}`,
                        mode === 'new_tab' ? 'New tab' : 'Same tab'
                      )}
                    </ToggleGroupItem>
                  ))}
                </ToggleGroup>
          ...
          <>
            ...
            <div className="pb-2">
              <div className="col d-flex justify-content-between align-items-center">
                <label className="form-label font-weight-400 mb-0">{t('editor.pageMenu.labels.selectApp', 'Select app')}</label>
                <Select
                  options={appOptions}
                  search={true}
                  value={page?.appId}
                  ...
                  placeholder={t('editor.pageMenu.selectPlaceholder', 'Select...')}
                  useMenuPortal={false}
                  width={'168px'}
            ...
            <div className="d-flex justify-content-between align-items-center pb-2">
              <label className="form-label font-weight-400 mb-0">
                {t('editor.pageMenu.labels.openAppIn', 'Open app in')}
              </label>
              <div className="ms-auto position-relative app-mode-switch" style={{ paddingLeft: '0px' }}>
                <ToggleGroup
                  onValueChange={(value) => {
...
const PageEvents = ({ page, allPages }) => {
  ...
  return (
    <div className="page-events">
      <div className="section-header pb-2">{t('editor.pageMenu.sections.pageEvents', 'Page events')}</div>
      <div>
        <EventManager
          component={{
            component: {
      ...
      <div className={cx('d-flex align-items-center justify-content-between')}>
        <div className={`field`}>
          <InspectorTooltip
            label={t(
              page?.type === 'default'
                ? 'editor.pageMenu.labels.hideThisPageOnNavigation'
                : 'editor.pageMenu.labels.hideThisItemOnNavigation',
              page?.type === 'default' ? 'Hide this page on navigation' : 'Hide this item on navigation'
            )}
