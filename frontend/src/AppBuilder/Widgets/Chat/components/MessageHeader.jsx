...
export const MessageHeader = ({ type, userName, respondentName, timestamp, onCopy, onDelete }) => (
  <div className="d-flex flex-row custom-gap-16 align-items-center justify-content-between">
    <div className="d-flex flex-row custom-gap-16">
      <span className="tj-text tj-header-h8 message-title">{type === 'message' ? userName : respondentName}</span>
      <span className="tj-text tj-text-xsm message-timestamp">{formatTimestamp(timestamp)}</span>
    </div>
