...
export default function autoGenerateFields(
  ...
  // Keep user-created fields, and autogenerated ones whose key still exists in data
  // Deletion history only gates generation, so a field and its duplicate can be deleted independently
  const persistingFields = existingFields.filter(
    (field) => !field?.autogenerated || keysOfData.includes(field.key || field.name)
  );
