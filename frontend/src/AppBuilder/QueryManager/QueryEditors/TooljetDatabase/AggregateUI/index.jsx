...
export const AggregateFilter = ({ darkMode, operation = "" }) => {
  ...
  const [
    currentAggregateKeyForDeleteConfirmation,
    setCurrentAggregateKeyForDeleteConfirmation,
  ] = useState(null);
  ...
  const showConfirmationMoalForLastFilledValue = (
    currentAggregates,
    aggregateKey
  ) => {
    ...
    const allValuesTruthy = Object.values(
      currentAggregates[aggregateKey]
  ...
  const handleDeleteAggregate = (aggregateKey) => {
    ...
    const showConfirmationModal = showConfirmationMoalForLastFilledValue(
      currentAggregates,
      aggregateKey
    );

    const isValidGroupByPresent = Object.values(currentGroupBy).some(
      (selectedColumn) => selectedColumn.length >= 1
    );
    ...
    const showError = () => {
      return toast.error(
        "Could not delete aggregate function. Please try again!"
      );
    };
  ...
  const handleGroupByChange = (selectedTableId, value) => {
    ...
    const validValueData = value?.reduce((acc, val) => {
      if (
        typeof val === "object" &&
        !acc.some((option) => option === val.value)
      ) {
        acc.push(val.value);
      } else if (
        typeof val !== "object" &&
  ...
  const getListRowsOption = (value) => {
    const option = columnAccessorsOptions?.find(
      ...
      <div className="d-flex" style={{ marginBottom: "1.5rem" }}>
        ...
        >
          ...
          {operationDetails?.aggregates &&
            ...
            Object.entries(operationDetails.aggregates).map(
              ([aggregateKey, aggregateDetails]) => {
                return (
                  <div key={aggregateKey} className="d-flex flex-row ">
                    ...
                    >
                      <SelectBox
                        width="25%"
                        height="32"
                        value={constructAggregateValue(
                          aggregateDetails.aggFx,
                          operation,
                          "aggFx"
                        )}
                    ...
                    <div
                      style={{ flex: "1" }}
                      className="border border-width-except-right"
                    >
                      <SelectBox
                        height="32"
                        width="100%"
          ...
          {operation === "listRows" && (
            <div className="border rounded ">
              <SelectBox
                ...
                isMulti={true}
                handleChange={(value) =>
                  handleGroupByChange(selectedTableId, value)
                }
              ...
              {joinTableOptions?.joins?.map((table) => {
                if (table.hasOwnProperty("table") && table.table) {
                  ...
                  const isTextTruncated =
                    isGroupByTableNameTruncated(tableName);
                  const tableNameEmpty = !tableName;
                  const showTooltip = tableNameEmpty || isTextTruncated;
                  const toolTipMessage = tableNameEmpty
                    ? "Please select joining table to see its name"
                    : tableName;
                  return (
                    <div key={table.table} className="border rounded d-flex">
                      ...
                      <div style={{ width: "75%" }}>
                        <SelectBox
                          ...
                          isMulti={true}
                          handleChange={(value) =>
                            handleGroupByChange(table.table, value)
                          }
