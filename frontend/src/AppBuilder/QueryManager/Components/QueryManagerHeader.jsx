...
export const QueryManagerHeader = forwardRef(
  ({ darkMode, setActiveTab, activeTab }, ref) => {
    const { moduleId } = useModuleContext();
    const updateQuerySuggestions = useStore(
      (state) => state.queryPanel.updateQuerySuggestions
    );
    ...
    const checkExistingQueryName = useStore(
      (state) => state.dataQuery.checkExistingQueryName
    );
    const selectedQuery = useStore((state) => state.queryPanel.selectedQuery);
    const selectedDataSource = useStore(
      (state) => state.queryPanel.selectedDataSource
    );
    const showCreateQuery = useStore(
      (state) => state.queryPanel.showCreateQuery
    );
    const setShowCreateQuery = useStore(
      (state) => state.queryPanel.setShowCreateQuery
    );
    ...
    const previewButtonOnClick = () => {
      ...
      posthogHelper.captureEvent("click_preview", {
    ...
    const tabs = [
      { id: 1, label: "Setup" },
      {
        ...
        condition: !["runpy", "runjs", "workflows"].includes(
          selectedQuery?.kind
        ),
      },
      { id: 3, label: "Settings" },
      ...
      <div className="row header" style={{ padding: "8px 16px" }}>
        <div className="col font-weight-500 p-0">
          ...
          {selectedQuery && (
            <div
              className="d-flex"
              style={{ marginBottom: "-15px", gap: "3px" }}
            >
              {tabs.map(
                (tab) =>
                  (tab.condition === undefined || tab.condition) && (
                    <p
                      ...
                      style={{
                        borderBottom:
                          activeTab === tab.id ? "2px solid #3E63DD" : "",
                        cursor: "pointer",
...
const NameInput = ({ onInput, value, darkMode, isDiabled, selectedQuery }) => {
  ...
  const isFocused = useStore(
    (state) => state.queryPanel.nameInputFocused,
    shallow
  );
  const setIsFocused = useStore(
    (state) => state.queryPanel.setNameInputFocused,
    shallow
  );
  const [name, setName] = useState(value);
  const selectedDataSourceScope = useStore(
    (state) => state.queryPanel.selectedDataSource?.scope,
    shallow
  );
      ...
      >
        {isFocused ? (
          <input
            ...
            type="text"
            className={cx(
              "border-indigo-09 bg-transparent query-rename-input py-1 px-2 rounded",
              {
                "text-white": darkMode,
              }
            )}
...
const RunButton = ({ buttonLoadingState }) => {
  ...
  const isLoading = useStore(
    (state) =>
      state.resolvedStore.modules.canvas.exposedValues.queries[
        selectedQuery?.id
      ]?.isLoading ?? false
  );
  const isMac =
  ...
  return (
    <span>
      ...
      >
        ...
        >
          Run
          <span className="query-manager-btn-shortcut">
            {isMac ? "⌘↩" : "Ctrl+Enter"}
          </span>
        </ButtonComponent>
      </ToolTip>
  ...
  const hasPermissions =
    ...
    selectedDataSource?.type !== DATA_SOURCE_TYPE.SAMPLE
      ? canUpdateDataSource(selectedQuery?.data_source_id) ||
        ...
        canDeleteDataSource()
      : true;
  const isPreviewQueryLoading = useStore(
    (state) => state.queryPanel.isPreviewQueryLoading
  );
