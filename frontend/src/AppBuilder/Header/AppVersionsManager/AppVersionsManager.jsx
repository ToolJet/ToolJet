...
export const AppVersionsManager = ({ darkMode, disabled = false }) => {
  ...
  const options = visibleVersions.map((appVersion) => {
    const versionDisplayName = appVersion.display_name || appVersion.displayName || appVersion.name;
    return {
      value: appVersion.id,
      isReleasedVersion: appVersion.id === releasedVersionId,
      appVersionName: versionDisplayName,
      label: (
          ...
          <div className="col-10">
            <ToolTip message="Current released version" show={appVersion.id === releasedVersionId} placement="right">
              <div
                ...
                style={{ maxWidth: '100%' }}
              >
                {decodeEntities(versionDisplayName)}
              </div>
            </ToolTip>
          </div>
