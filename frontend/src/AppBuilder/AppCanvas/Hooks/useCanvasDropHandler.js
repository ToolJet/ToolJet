...
export const useCanvasDropHandler = () => {
  ...
  const handleDrop = async ({ componentType: draggedComponentType, component }, canvasId) => {
    ...
    if (dropComponent?.moduleId && dropComponent?.isStub && dropComponent?.appId) {
      const toastId = toast.loading('Loading module..');
      try {
        ...
        const editingVersion = hydrated.app_versions?.[0];
        if (!editingVersion || editingVersion.is_stub) {
        ...
        dropComponent = {
          ...dropComponent,
          isStub: false,
          versionId: editingVersion.name,
          environmentId: editingVersion.current_environment_id,
