...
if command -v nsjail >/dev/null 2>&1; then
  ...
  # Set default security mode to FULL
  # Users can override by setting PYTHON_SECURITY_MODE environment variable
  if [ -z "$PYTHON_SECURITY_MODE" ]; then
    export PYTHON_SECURITY_MODE="full"
    echo "   Security Mode: full (default)"
  elif [ "$PYTHON_SECURITY_MODE" = "auto" ]; then
    # Auto-detect mode if explicitly set to "auto"
    echo "🔍 Auto-detecting Python security mode..."
    ...
    # Check for unprivileged user namespaces
      ...
      export PYTHON_SECURITY_MODE="userns"
      echo "   ✅ USERNS mode: Unprivileged namespaces available"
    else
      export PYTHON_SECURITY_MODE="basic"
    ...
    echo "   Security Mode: ${PYTHON_SECURITY_MODE} (user-configured)"
  fi
