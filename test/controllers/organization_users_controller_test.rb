...
class OrganizationUsersControllerTest < ActionDispatch::IntegrationTest
    def setup
        ...
        @admin = User.create({first_name: "Admin", email: "admin@example.com", password: "password", organization: @org })
        @developer = User.create({first_name: "Dev", email: "dev@example.com", password: "password", organization: @org })
        @viewer = User.create({first_name: "Viewer", email: "viewer@example.com", password: "password", organization: @org })
        ...
        @another_org_admin = User.create({first_name: "Admin", email: "admin@domain.com", password: "password", organization: @another_org })
        @org_another_org_admin = OrganizationUser.create(organization: @another_org, user: @another_org_admin, role: 'admin')
    end
    ...
    test 'org admins can change role of org users' do 
        ...
        post organization_user_change_role_url(@org_user_developer.id), params: { role: 'viewer' }, as: :json, headers: auth_header(@admin)
        assert_response 204
    ...
    test 'cannot change role of org users if not org admin' do 
        assert_equal "developer", @org_user_developer.role
        post organization_user_change_role_url(@org_user_developer.id), params: { role: 'viewer' }, as: :json, headers: auth_header(@viewer)
        assert_response 403
        assert_equal "developer", @org_user_developer.reload.role

        assert_equal "viewer", @org_user_viewer.role
        post organization_user_change_role_url(@org_user_viewer.id), params: { role: 'viewer' }, as: :json, headers: auth_header(@developer)
        assert_response 403
    ...
    test 'org users of one org cannot change role of users of another org' do 
        ...
        post organization_user_change_role_url(@org_another_org_admin.id), params: { role: 'viewer' }, as: :json, headers: auth_header(@admin)
        assert_response 403
