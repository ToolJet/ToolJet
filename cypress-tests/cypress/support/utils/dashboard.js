...
export const verifyAppCardOptions = (appName, type='app') => {
  viewAppCardOptions(appName);
  cy.get('[data-cy="rename-app-card-option"]').verifyVisibleElement(
    "have.text",
    "Rename app"
  );
  cy.get(
...
export const verifyCloneApp = (appName, cloneAppName) => {
  ...
  cy.renameApp(cloneAppName);
  cy.apiAddComponentToApp(cloneAppName, "button", 25, 25);
...
export const verifyAppCrudOperations = (appName) => {
  const customLayout = {
    desktop: { top: 100, left: 20 },
    mobile: { width: 8, height: 50 },
  };
  cy.createApp(appName);
  cy.apiAddComponentToApp(appName, "text1", customLayout);
  cy.backToApps();
  cy.get(commonSelectors.appCard(appName)).should("contain.text", appName);
