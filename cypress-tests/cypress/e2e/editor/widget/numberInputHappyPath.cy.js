...
describe('Number Input', ()=>{
  ...
  it("should verify the properties of the number input widget", ()=>{
    ...
    openEditorSidebar(data.widgetName);
    openAccordion(commonWidgetText.accordionProperties);
    verifyAndModifyParameter(commonWidgetText.labelMinimumValue, data.minimumvalue);
    cy.get(commonWidgetSelector.buttonCloseEditorSideBar).click();
    cy.clearAndType(commonWidgetSelector.draggableWidget(data.widgetName), randomNumber(1,4))
    cy.get(commonWidgetSelector.draggableWidget(data.widgetName)).verifyVisibleElement("have.value",data.minimumvalue);


    openEditorSidebar(data.widgetName);
    openAccordion(commonWidgetText.accordionProperties);
    verifyAndModifyParameter(commonWidgetText.labelMaximumValue, data.maximumValue);
    cy.clearAndType(commonWidgetSelector.draggableWidget(data.widgetName), randomNumber(100,110))
    cy.get(commonWidgetSelector.draggableWidget(data.widgetName)).verifyVisibleElement("have.value", data.maximumValue);
    ...
    cy.get(commonWidgetSelector.widgetDocumentationLink).should("have.text",numberInputText.numberInputDocumentationLink);
  ...
  it("should verify the app preview", ()=>{
    ...
    cy.clearAndType(commonWidgetSelector.draggableWidget(numberInputText.defaultWidgetName), randomNumber(1,4))
    cy.get(commonWidgetSelector.draggableWidget(numberInputText.defaultWidgetName)).verifyVisibleElement("have.value",data.minimumvalue);
    cy.clearAndType(commonWidgetSelector.draggableWidget(numberInputText.defaultWidgetName), randomNumber(100,110))
    cy.get(commonWidgetSelector.draggableWidget(numberInputText.defaultWidgetName)).verifyVisibleElement("have.value", data.maximumValue);
    cy.get(commonWidgetSelector.draggableWidget(numberInputText.defaultWidgetName))
    .invoke('attr', 'placeholder').should('contain', data.randomNumber);
    
    verifyTooltip(commonWidgetSelector.draggableWidget(numberInputText.defaultWidgetName), data.tooltipText)
    cy.get(commonWidgetSelector.draggableWidget(commonWidgetText.text1)).verifyVisibleElement("have.text", data.maximumValue);
