/**
 * SPEC — Number Input — propertiesFx facet.
...
describe('Number Input — propertiesFx facet', { testIsolation: false }, () => {
    const W = 'numberinput1';

    beforeEach(() => {
        cy.apiLogin();
        cy.apiCreateApp(`${fake.companyName}-NIPropsFx-${Cypress._.uniqueId()}`);
        cy.openApp();
    ...
    it('value fx — {{binding}} resolves to the input value', () => {
        openEditorSidebar(W);
        // value (code, fxCapable). source: numberinput.js:27
    ...
    it('label fx — {{binding}} resolves to the label text', () => {
        openEditorSidebar(W);
        // label (code, fxCapable). source: numberinput.js:22
    ...
    it('tooltip fx — code field accepts a binding', () => {
        ...
        // tooltip (code, fxCapable). source: numberinput.js:96
    ...
    it('mandatory fx — verify {{false}} default → flip ON → * marker', () => {
        ...
        verifyAndModifyToggleFx('Make this field mandatory', '{{false}}'); // flips ON. source: numberinput.js:327
        cy.forceClickOnCanvas();
