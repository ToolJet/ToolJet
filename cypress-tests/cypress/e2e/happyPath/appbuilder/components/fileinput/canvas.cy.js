...
describe(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      cy.openApp();
      cy.dragAndDropWidget(fileInputText.defaultWidgetText, dropX, dropY);
    ...
    it("drag-and-drop places the widget where it was dropped", () => {
      ...
      cy.get("#real-canvas").then(($canvas) => {
        ...
        cy.get(fileInputSelector.draggableWidget(widget)).then(($widget) => {
    ...
    //
    // Not a File Input issue and not a helper bug: checkbox, numberInput, passwordInput
    ...
    it("multi-select then select-all", () => {
      ...
      cy.dragAndDropWidget(fileInputText.defaultWidgetText, 650, 300);
      waitForDropSettle("fileinput2");
      closeQueryPanel();

      multiSelectWidgets([widget, "fileinput2"]);
    ...
    it("rename via the component menu", () => {
      ...
      cy.get(fileInputSelector.draggableWidget(widget)).should("not.exist");
    });
