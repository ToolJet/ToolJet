...
// Covers both config.others items — source: filepicker.js:11-12
//   showOnDesktop:11 (default true) · showOnMobile:12 (default false)
// These gate whether the widget mounts per layout at all — a different code path from the
...
describe(
  ...
  () => {
    ...
    it("device context — show on desktop and show on mobile gate the widget per layout", () => {
      ...
      cy.get(filePickerSelector.widget(widget)).should("exist");

      switchLayout("mobile");
      cy.get(filePickerSelector.widget(widget)).should("not.exist");

      // Enable mobile from the DESKTOP layout: once hidden, the widget can't be selected
      // to reach its Inspector.
