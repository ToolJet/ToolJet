...
describe('Password Input — csa facet', { testIsolation: false }, () => {
    const W = "passwordinput1";

    beforeEach(() => {
        cy.apiLogin();
        cy.apiCreateApp(`${fake.companyName}-PICsa-${Cypress._.uniqueId()}`);
        cy.openApp();
        cy.dragAndDropWidget("Password Input", 500, 100);
        cy.get('[data-cy="query-manager-toggle-button"]').click();
    });

    it('should verify all the CSA from the password input (On click)', () => {
        const actions = [
            { event: "On click", action: "Set visibility", valueToggle: "{{false}}" }, // b1 source: passwordinput.js:305
            { event: "On click", action: "Set visibility", valueToggle: "{{true}}" },  // b2 source: passwordinput.js:305
            { event: "On click", action: "Set disable", valueToggle: "{{true}}" },     // b3 source: passwordinput.js:310
            { event: "On click", action: "Set disable", valueToggle: "{{false}}" },    // b4 source: passwordinput.js:310
            { event: "On click", action: "Set text", value: "100" },                   // b5 source: passwordinput.js:288
            { event: "On click", action: "Clear" },                                    // b6 source: passwordinput.js:293
            { event: "On click", action: "Set focus" },                                // b7 source: passwordinput.js:297
            { event: "On click", action: "Set blur" },                                 // b8 source: passwordinput.js:301
            { event: "On click", action: "Set loading", valueToggle: "{{true}}" },     // b9 source: passwordinput.js:315
        ];
        addCSA(W, actions);
        ...
        cy.get(btn(5)).click();
        cy.get(input).should("have.value", "100"); // source: passwordinput.js:288
        cy.get(btn(6)).click();
        ...
        cy.get(btn(7)).click(); // source: passwordinput.js:297
        cy.get(btn(8)).click(); // source: passwordinput.js:301
