/**
 * SPEC — Password Input — events facet.
...
describe('Password Input — events facet', { testIsolation: false }, () => {
    const W = 'passwordinput1';

    beforeEach(() => {
        cy.apiLogin();
        cy.apiCreateApp(`${fake.companyName}-PIEvents-${Cypress._.uniqueId()}`);
        cy.openApp();
    ...
    it('should verify all the events from the password input', () => {
        openEditorSidebar(W);
        // events: onChange, onFocus, onBlur, onEnterPressed. source: passwordinput.js:112-115
        const events = [
            { event: 'On focus', message: 'On focus Event' },              // source: passwordinput.js:113
            { event: 'On blur', message: 'On blur Event' },                // source: passwordinput.js:114
            { event: 'On change', message: 'On change Event' },            // source: passwordinput.js:112
            { event: 'On enter pressed', message: 'On enter pressed Event' }, // source: passwordinput.js:115
        ];
