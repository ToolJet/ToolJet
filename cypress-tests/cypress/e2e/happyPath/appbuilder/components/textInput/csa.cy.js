...
describe('Text Input — csa facet', { testIsolation: false }, () => {
    const W = "textinput1";

    beforeEach(() => {
        cy.apiLogin();
        cy.apiCreateApp(`${fake.companyName}-TICsa-${Cypress._.uniqueId()}`);
        cy.openApp();
        cy.dragAndDropWidget("Text Input", 500, 100);
        cy.get('[data-cy="query-manager-toggle-button"]').click();
    });

    it('should verify all the CSA from the text input (On click)', () => {
        const actions = [
            { event: "On click", action: "Set visibility", valueToggle: "{{false}}" }, // b1 source: textinput.js:305
            { event: "On click", action: "Set visibility", valueToggle: "{{true}}" },  // b2 source: textinput.js:305
            { event: "On click", action: "Set disable", valueToggle: "{{true}}" },     // b3 source: textinput.js:310
            { event: "On click", action: "Set disable", valueToggle: "{{false}}" },    // b4 source: textinput.js:310
            { event: "On click", action: "Set text", value: "100" },                   // b5 source: textinput.js:288
            { event: "On click", action: "Clear" },                                    // b6 source: textinput.js:293
            { event: "On click", action: "Set focus" },                                // b7 source: textinput.js:297
            { event: "On click", action: "Set blur" },                                 // b8 source: textinput.js:301
            { event: "On click", action: "Set loading", valueToggle: "{{true}}" },     // b9 source: textinput.js:315
        ];
        addCSA(W, actions);
        ...
        cy.get(btn(5)).click();
        cy.get(input).should("have.value", "100"); // source: textinput.js:288
        cy.get(btn(6)).click();
        ...
        cy.get(btn(7)).click(); // source: textinput.js:297
        cy.get(btn(8)).click(); // source: textinput.js:301
