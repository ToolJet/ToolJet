    ...
    it.skip("should validate Export App API flows", () => {
        ...
        fetchWorkspaceApps(workspaceId).then((response) => {
            ...
            exportApp(workspaceId, appId, "").then((response) => {
                expect(response.status).to.eq(201);
                expect(response.body.app, "app array in response").to.be.an("array").and.not.be.empty;
                expect(
            ...
            exportApp(workspaceId, appId, "?exportAllVersions=true").then(
                (response) => {
                    ...
                    const versionNames = response.body.app[0].definition.appV2.appVersions.map(v => v.name);
                    expect(versionNames, "should contain v1, v2, v3").to.have.members(["v1", "v2", "v3"]);
                }
            );
            ...
            exportApp(workspaceId, appId, "?exportTJDB=false").then((response) => {
                expect(response.status).to.eq(201);
                expect(response.body, "should not include tooljet_database").not.to.have.property("tooljet_database");
            });

            // Act & Assert: Export with ToolJet Database
            exportApp(workspaceId, appId, "?exportTJDB=true").then((response) => {
                expect(response.status).to.eq(201);
                expect(response.body, "should include tooljet_database").to.have.property("tooljet_database");
            });
            ...
            exportApp(`${workspaceId}invalid`, appId, "").then((response) => {
                expect(response.status, "invalid workspace ID should return 400 or 404").to.be.oneOf([400, 404]);
            });
        });
