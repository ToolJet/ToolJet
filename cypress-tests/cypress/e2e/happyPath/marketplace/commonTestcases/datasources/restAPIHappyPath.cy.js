...
import { restAPIText } from "Texts/marketplace/restAPI";
import { createAndRunRestAPIQuery } from "Support/utils/marketplace/dataSource/restAPI";

const data = {};
const authenticationDropdownSelector =
  '[data-cy="authentication-type-dropdown-label"] ~ div .react-select__control';
const grantTypeDropdown =
  '[data-cy="grant-type-section"] .react-select__control';
const addAccessTokenDropdown =
  '[data-cy="add-access-token-to-section"] .react-select__control';
const clientAuthenticationDropdown =
  '[data-cy="client-authentication-section"] .react-select__control';
...
describe("Data source Rest API", () => {
  ...
  it("Should verify elements on Rest API connection form", () => {
    ...
    cy.get('[data-cy="databases-datasource-button"]').should(
      "have.text",
      Cypress.env("marketplace_action") ? "Databases (17)" : "Databases (20)"
    );
    ...
    sections.forEach((section) => {
      if (section !== restAPIText.baseUrlLabel) {
        cy.get(restAPISelector.subSection(section)).find(`[data-cy="${section.toLowerCase().replace(/\s+/g, "-")}-label"]`).should(
          "have.text",
          section
        );
      }
  ...
  it("Should verify connection response for all methods", () => {
    ...
    data.curdAppName = `${fake.companyName}-restAPI-CURD-App`;
    cy.apiCreateApp(data.curdAppName);
    cy.openApp();
