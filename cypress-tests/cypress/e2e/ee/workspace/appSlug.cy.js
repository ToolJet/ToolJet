...
import { logout, navigateToAppEditor, verifyTooltip } from "Support/utils/common";
...
describe("App slug", () => {
    ...
    it("Verify app slug cases in global settings", () => {
        ...
        cy.url().should('eq', `http://localhost:8082/applications/${data.slug}/home?env=development&version=v1`)
        cy.go('back');
        cy.wait(2000);

        promoteApp();
        cy.openInCurrentTab(commonWidgetSelector.previewButton);
        cy.url().should('eq', `http://localhost:8082/applications/${data.slug}/home?env=staging&version=v1`)
        cy.go('back');
        cy.wait(2000);

        promoteApp();
        cy.openInCurrentTab(commonWidgetSelector.previewButton);
        cy.url().should('eq', `http://localhost:8082/applications/${data.slug}/home?env=production&version=v1`)
        cy.go('back');
        cy.wait(2000);
        ...
        cy.url().should('eq', `http://localhost:8082/applications/${data.slug}/home`);
