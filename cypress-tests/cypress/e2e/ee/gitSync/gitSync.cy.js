import { insertGitSyncSSHKey, insertGitSyncSSHSecondKey } from "Support/utils/eeCommon";
...
describe("", () => {
    ...
    it("Verify git push and pull functionalities", () => {
        ...
        cy.clearAndType('.template-search-box > .input-icon > .form-control', "Inventory management - PostgreSQL");
        cy.get('.template-app-list > [data-cy="-list-item"]').click();
        ...
        cy.get(gitSyncSelector.commitHelperText).verifyVisibleElement("have.text", "This will commit the creation of the new version to the git repo")
        ...
        cy.wait(3000);
        cy.get('[data-cy="draggable-widget-text35"]').click();
        verifyAndModifyParameter("Text", "Git sync testing");
        cy.forceClickOnCanvas();
        cy.waitForAutoSave();

        cy.get(commonEeSelectors.gitSyncIcon).click();
        cy.get(gitSyncSelector.lastCommitInput).verifyVisibleElement("have.text", "Version Creation")
        cy.get(gitSyncSelector.lastCommitVersion).verifyVisibleElement("have.text", "v2")
        cy.get(gitSyncSelector.autherInfo).should("include.text", "Done by The Developer at")
        cy.clearAndType(gitSyncSelector.commitMessageInput, "Git sync test commit");
        cy.get(commonSelectors.cancelButton).click();

        cy.get(commonEeSelectors.gitSyncIcon).click();
        cy.get(gitSyncSelector.commitMessageInput).verifyVisibleElement("have.attr", "placeholder", "Briefly describe the changes you've made")
        cy.clearAndType(gitSyncSelector.commitMessageInput, "Git sync test commit");
        ...
        cy.wait(3000);
        cy.get(commonEeSelectors.modalTitle).verifyVisibleElement("have.text", "Commit changes")
        cy.get(commonEeSelectors.modalCloseButton).should('be.visible')
        cy.get(commonSelectors.modalMessage).verifyVisibleElement("have.text", "Commiting changes to v3 will replace v2 in the git repo. Are you sure you want to continue?")
        cy.get(commonSelectors.cancelButton).verifyVisibleElement("have.text", "Cancel")
        cy.get(commonEeSelectors.confirmButton).verifyVisibleElement("have.text", "Continue")
        cy.get(commonEeSelectors.confirmButton).click();
        cy.verifyToastMessage(commonSelectors.toastMessage, "Changes commited successfully")
        cy.wait(3000)
        ...
        cy.get(commonEeSelectors.modalTitle).verifyVisibleElement("have.text", "Import app from git repository")
        cy.get(commonEeSelectors.modalCloseButton).should('be.visible')
        cy.verifyLabel("Create app from")
        cy.get('#react-select-3-placeholder').should('be.visible')
        cy.get(commonSelectors.cancelButton).verifyVisibleElement("have.text", "Cancel")
        cy.get(commonEeSelectors.confirmButton).verifyVisibleElement("have.text", "Import app")
        cy.get('.react-select__input-container').click()
        cy.get('.react-select__input-container').type(data.appName)
        cy.wait(1000)
        ...
        cy.get(gitSyncSelector.appNameField).verifyVisibleElement("have.value", data.appName)
        cy.verifyLabel("Last commit")
        cy.get(gitSyncSelector.lastCommitInput).verifyVisibleElement("have.text", "Git sync test commit")
        cy.get(gitSyncSelector.lastCommitVersion).verifyVisibleElement("have.text", "v3")
        cy.get(gitSyncSelector.autherInfo).should("include.text", "Done by The Developer at")
        cy.get(commonSelectors.cancelButton).verifyVisibleElement("have.text", "Cancel")
        cy.get(commonEeSelectors.confirmButton).verifyVisibleElement("have.text", "Import app")
        cy.get(commonEeSelectors.confirmButton).click();
        ...
        cy.get(commonSelectors.warningText).verifyVisibleElement("have.text", "Apps imported from git repository cannot be edited")
        ...
        cy.get(commonEeSelectors.gitSyncIcon).click();
        cy.wait(2000);
        cy.get(commonEeSelectors.modalTitle).verifyVisibleElement("have.text", "GitSync")
        cy.get(commonEeSelectors.modalCloseButton).should('be.visible')
        cy.verifyLabel("Git repo URL")
        cy.get(gitSyncSelector.gitRepoInfo).verifyVisibleElement("have.value", "git@github.com:ajith-k-v/test.git")
        cy.verifyLabel("Check for updates")
        cy.get(commonSelectors.cancelButton).verifyVisibleElement("have.text", "Cancel")
        cy.get(gitSyncSelector.pullButton).verifyVisibleElement("have.text", "Pull changes")
        cy.get(commonSelectors.cancelButton).click();
        ...
        cy.get('[data-cy="draggable-widget-text35"]').verifyVisibleElement("have.text", "Git sync testing")
        cy.visit(`${data.workspaceName}/data-sources`);
        cy.wait(2000);
        cy.get('[data-cy="postgresql---inventory-management-button"] > .font-400').should("be.visible")
    })
    ...
    it("Verify git sync modal UI", () => {
        ...
        cy.verifyLabel("Commit changes")
        cy.get(gitSyncSelector.commitHelperText).verifyVisibleElement("have.text", "This action commits the app's creation to the git repository")
        ...
        cy.get(commonEeSelectors.modalCloseButton).should('be.visible')
        cy.verifyLabel("Git repo URL");
        cy.get(gitSyncSelector.gitRepoInput).verifyVisibleElement("have.value", "git@github.com:ajith-k-v/test.git")
        cy.verifyLabel("Commit message");
        cy.get(gitSyncSelector.commitMessageInput).verifyVisibleElement("have.attr", "placeholder", "Briefly describe the changes you've made")
        cy.verifyLabel("Last commit");
        cy.get(gitSyncSelector.lastCommitInput).verifyVisibleElement("have.text", "App creation")
        cy.get(gitSyncSelector.lastCommitVersion).verifyVisibleElement("have.text", "v1")
        cy.get(gitSyncSelector.autherInfo).should("include.text", "Done by The Developer at")

        cy.get(commonSelectors.cancelButton).verifyVisibleElement("have.text", "Cancel")
        cy.get(gitSyncSelector.commitButton).verifyVisibleElement("have.text", "Commit changes")
        ...
        cy.get(gitSyncSelector.gitSyncApphelperText).verifyVisibleElement("have.text", "Sync applications to your git repository and never lose your progress!")
        cy.get(gitSyncSelector.connectRepoButton).verifyVisibleElement("have.text", "Connect to repository");
