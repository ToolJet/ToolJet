...
describe("Manage Users for multiple workspace", ()=>{
 ...
 it("Should verify the Manage users page", ()=>{
  ...
  cy.get(usersSelector.fisrtNameError).verifyVisibleElement("have.text", usersText.fieldRequired);
  cy.get(usersSelector.lastNameError).verifyVisibleElement("have.text", usersText.fieldRequired);
  cy.get(usersSelector.emailError).verifyVisibleElement("have.text", usersText.fieldRequired);
 ...
 it("should verify the new user account", ()=>{
  ...
  cy.intercept("GET", "/assets/images/onboarding.svg").as('emptyDashboardImage')
  ...
  cy.contains('td', data.email).parent().within(() => {
 ...
 it("Should verify the archive functionality",()=>{
  ...
  cy.verifyToastMessage(commonSelectors.toastMessage,usersText.unarchivedToast);
