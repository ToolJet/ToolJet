...
import { navigateToManageGroups, verifyTooltipDisabled, viewAppCardOptions } from "Support/utils/common";
import { OpenGroupCardOption, verifyGroupCardOptions, createGroupAddAppAndUserToGroup, groupPermission } from "Support/utils/manageGroups";
...
describe("Groups duplication", () => {
    beforeEach(() => {
        ...
        groupPermission(["appsCreateCheck", "appsDeleteCheck", "workspaceVarCheckbox", "foldersCreateCheck"], 'All users')
    ...
    it("Should verify the group duplication feature", () => {
        ...
        cy.get('[data-cy="delete-group-card-option"] > .col').should("have.class", "disable");

        verifyGroupCardOptions("Admin")
        cy.get('[data-cy="delete-group-card-option"] > .col').should("have.class", "disable");

        cy.apiCreateApp(data.appName);
        createGroupAddAppAndUserToGroup(groupName, data.email)

        groupPermission(["appsCreateCheck", "appsDeleteCheck", "workspaceVarCheckbox", "foldersCreateCheck"], groupName, true)
        cy.wait(1000);
        verifyGroupCardOptions(groupName)
        cy.get(groupsSelector.duplicateOption).click()

        cy.get(commonSelectors.defaultModalTitle).verifyVisibleElement("have.text", "Duplicate group")
        cy.get(commonSelectors.modalMessage).verifyVisibleElement("have.text", "Duplicate the following parts of the group")
        ...
        cy.get(groupsSelector.confimButton).verifyVisibleElement("have.text", "Duplicate")

        cy.get(groupsSelector.confimButton).click()
        cy.verifyToastMessage(commonSelectors.toastMessage, "Group duplicated successfully!")

        cy.wait(500);
        cy.get(groupsSelector.duplicatedGroupLink(groupName)).verifyVisibleElement("have.text", `${groupName}_copy`)
        ...
        cy.defaultWorkspaceLogin();
        navigateToManageGroups();
        OpenGroupCardOption(`${groupName}_copy`);
        cy.get(groupsSelector.deleteGroupOption).click();
        cy.get(commonSelectors.buttonSelector("Yes")).click();
        cy.logoutApi();
