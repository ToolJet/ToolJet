 ...
 * VERSION row per (app_id, branch_id), with no exception for unsynced apps.
 *
 ...
 *
 * Scoping both partial indexes to `is_synced = TRUE` keeps the single-draft
 * invariant for synced apps (git sync's normal operating mode) while exempting
 * unsynced apps entirely — matching the pre-git-sync workspace behavior.
 */
