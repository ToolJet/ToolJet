...
describe('workflow executions controller', () => {
  ...
  describe('POST /api/workflow_executions', () => {
    ...
    it('should not allow users who does not have permission, to create new workflow execution', async () => {
      const userData = await createUser(app, { email: 'admin@tooljet.io' });
      const { user } = userData;
      const { user: anotherUser } = await createUser(app, { email: 'another@tooljet.io' });

      const workflow = await createApplication(app, { name: 'workflow', anotherUser, type: 'workflow' });
      await createApplicationVersion(app, workflow, {
        definition: {
          nodes: [
            {
