...
describe('Form Authentication', () => {
  ...
  describe('Multi Organization Operations', () => {
    ...
    describe('Signup Instance admin and invite users', () => {
      describe('Signup instance admin', () => {
        ...
        it('should verify invitation token of instance admin', async () => {
          const { body } = await verifyInviteToken(current_user);
          expect(body?.email).toEqual('admin@tooljet.com');
          expect(body?.name).toEqual('Admin');
        });

        it('should return user info and setup instance admin account using invitation token (setup-account-from-token)', async () => {
        ...
        it('should allow instance admin to view apps', async () => {
          const response = await request(app.getHttpServer())
            .get(`/api/apps`)
