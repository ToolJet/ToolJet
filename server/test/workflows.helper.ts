...
export const setupOrganizationAndUser = async (
  ...
  await updateInstanceSetting(
    nestApp,
    INSTANCE_USER_SETTINGS.ALLOW_PERSONAL_WORKSPACE,
    allowPersonalWorkspace.toString()
  );
...
const updateInstanceSetting = async (nestApp: INestApplication, key: string, value: string): Promise<void> => {
  const defaultDataSource = nestApp.get<TypeOrmDataSource>(getDataSourceToken('default'));
...
export const createNestAppInstance = async (
  options: {
    edition?: 'ce' | 'ee' | 'cloud';
