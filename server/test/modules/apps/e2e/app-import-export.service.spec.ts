  ...
  describe('EE (plan: enterprise)', () => {
    ...
    describe('.import | deserialize and create app from payload', () => {
      it('should throw error with invalid params', async () => {
        ...
        await expect(service.import(adminUser, 'hello world', appName)).rejects.toThrow(
          'Invalid params for app import'
        );
      });
      ...
      it.skip('should import app with related associations', async () => {
        ...
        const adminUser = adminUserData.user;
        const { application, appVersion: applicationVersion } = await createAppWithDependencies(
          nestApp,
          adminUserData.user,
          {
            isDataSourceNeeded: false,
    ...
    describe('.import | slug isolation', () => {
      ...
      it('should produce unique slugs when importing the same export twice', async () => {
        ...
        // Each import gets its own fresh random-UUID slug placeholder, so the two are distinct.
        expect(firstRecord.appVersions[0].slug).toMatch(UUID_PATTERN);
        expect(secondRecord.appVersions[0].slug).toMatch(UUID_PATTERN);
    ...
    describe('.import | globalSettings preservation', () => {
      it('should preserve globalSettings from the exported version', async () => {
        ...
        const { newApp } = await service.import(
          adminUser,
          exportedApp,
          'global-settings-preservation-test',
          {},
          false,
          '3.0.0'
        );
