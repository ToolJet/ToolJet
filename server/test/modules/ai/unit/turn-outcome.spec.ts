...
describe('classifying how an agent turn concluded', () => {
  ...
  it('reads rejected provider credentials the same way', () => {
    expect(classifyTurn({ ...completed, agentErrored: true, errorCategory: 'provider_config' })).toBe('awaiting_input');
  });
  ...
  it('puts a user cancellation ahead of everything', () => {
    expect(classifyTurn({ ...completed, cancelled: true, agentErrored: true, hasPendingInterrupt: true })).toBe(
      'cancelled'
    );
  });
});
