  ...
  describe('EE (plan: enterprise)', () => {
    ...
    describe('App git life cycle', () => {
      ...
      it('should complete the full app git life cycle', async () => {
        ...
        //     the only remaining on-open hydration trigger), repoint the
        ...
        // Return the first datasource's name — the DIRECTORY name (the name is no longer
        // stored inside data-source.json).
        ...
        // 59. Same shape as step 55 for data sources. The DS NAME is the DIRECTORY
        //     (data-sources/<name>/) — no `name` is stored in the file — so a name
        //     collision is a directory whose name matches an existing DB data source but
        //     carries a DIFFERENT co_relation_id. Re-point the existing DS's directory at
        //     a fresh co_relation_id (git side only; the 409 blocks the pull, so the DB DS
        //     keeps its real id → same name, different identity → conflict), then restore.
        ...
        // The orphan was faked via a direct DB write (git HEAD unchanged), so clear
        // this branch's git-sync skip tokens to force the pull to fully re-examine
        // git — simulating the git change that produces an orphan in a real flow.
        await dataSource.query(
          `UPDATE organization_git_sync_branches
           SET last_synced_commit = NULL, apps_git_tree_sha = NULL, modules_git_tree_sha = NULL, data_sources_git_tree_sha = NULL
           WHERE id = $1`,
    ...
    describe('POST /api/v2/resources/import | multi-version import respects git sync', () => {
      ...
      beforeAll(async () => {
        ...
        const { user, organization } = await createUser(app, {
          email: 'import-versions-src@tooljet.io',
          firstName: 'import',
          lastName: 'source',
        });
        const { tokenCookie: srcCookie } = await login(app, 'import-versions-src@tooljet.io');
      ...
      it('imports ALL versions when git sync is disabled', async () => {
        const { organization } = await createUser(app, {
          email: 'import-versions-gitoff@tooljet.io',
          firstName: 'import',
          lastName: 'gitoff',
        });
        const { tokenCookie: cookie } = await login(app, 'import-versions-gitoff@tooljet.io');
        await ensureAppEnvironments(app, organization.id);
      ...
      it('imports ONLY the latest version when git sync is enabled', async () => {
        const { organization } = await createUser(app, {
          email: 'import-versions-giton@tooljet.io',
          firstName: 'import',
          lastName: 'giton',
        });
        const { tokenCookie: cookie } = await login(app, 'import-versions-giton@tooljet.io');
        await ensureAppEnvironments(app, organization.id);

        // Enable git sync for this workspace — flips getDetails().isEnabled to true
        // (hits the real Gitea/GitHub-Enterprise server, same as the rest of this suite).
    ...
    describe('pull re-marks resources synced after is_synced reset (git disable→enable)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-resync@tooljet.io',
          firstName: 'git',
          lastName: 'resync',
        });
        syncOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-resync@tooljet.io');
    ...
    describe('changing the repo URL resets the default branch sync state', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-url-change@tooljet.io',
          firstName: 'git',
          lastName: 'urlchange',
        });
        urlOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-url-change@tooljet.io');
      ...
      it('flips is_synced=false and clears last_synced_commit on the default branch when the git URL changes', async () => {
        ...
        // ── 4. change the repo URL (point at a different, freshly-reset repo) ────────────
        step(4, 'reset the new repo, then save configs with a different gitUrl');
        ...
        await auth(agent().post('/api/git-sync/configs'))
          .send({ ...GITHUB_HTTPS_PAYLOAD, gitUrl: NEW_REPO_URL, useEnvConfig: false })
          .expect(201);
    ...
    describe('feature-branch datasource push does not sync the default branch (regression)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-ds-crossbranch@tooljet.io',
          firstName: 'git',
          lastName: 'dscross',
        });
        dsxOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-ds-crossbranch@tooljet.io');
    ...
    describe('deleted data sources are not committed / are removed on push (regression)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-ds-delete@tooljet.io',
          firstName: 'git',
          lastName: 'dsdelete',
        });
        ddOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-ds-delete@tooljet.io');
      ...
      it('app push does NOT commit a data source that was deleted on the branch (serializeLinkedDataSourcesForApp)', async () => {
        ...
        // Mirror the real feature-branch delete (service.delete sets is_active=false). Done via
        // SQL because the linked query would block the API delete — the point here is that
        // serialize must not write a data source whose branch DSV is inactive, regardless.
        await ddDs.query(
    ...
    describe('cross-branch app push carries connected data sources and modules (regression)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-dep-carry@tooljet.io',
          firstName: 'git',
          lastName: 'depcarry',
        });
        depOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-dep-carry@tooljet.io');
    ...
    describe('slug update — git-sync branch rules (regression)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-slug-rules@tooljet.io',
          firstName: 'git',
          lastName: 'slugrules',
        });
        sgOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-slug-rules@tooljet.io');
    ...
    describe('single-branch lifecycle: push/pull apps, modules, data sources on the default branch', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-single-branch@tooljet.io',
          firstName: 'git',
          lastName: 'singlebranch',
        });
        sbOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-single-branch@tooljet.io');
        ...
        // finalize promotes/keeps it (rather than renaming a `main` default).
        await sbDs.query(
          `INSERT INTO organization_git_sync_branches (organization_id, branch_name, is_default)
      ...
      it('pushes and pulls apps, modules, and data sources directly on the (unprotected) single-branch default', async () => {
        ...
        try {
          ...
          // the app + module remain in git — only the deleted data source was removed
      ...
      // Data-source FOLDERS: a data source placed in a folder must serialize UNDER its folder
      // directory (data-sources/<folder>/<ds>/data-source.json), round-trip its branch-scoped
      // folder_data_sources membership on pull, and re-home (with the old folder dir pruned) when
      // moved to another folder. Exercises resolvePlacementFolderName (push), attachFoldersForDataSources
      // (pull) and pruneStaleDataSourceFolders (move) end to end against the real git server.
      it('serializes a foldered data source under data-sources/<folder>/<ds>/ and round-trips its branch-scoped membership', async () => {
        ...
        step(1, 'reset gitea + configure single-branch git, disable branching, pull baseline');
        await fetch(RESET_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: BASIC },
          body: '{}',
        ...
        const dsId: string = (
          await auth(agent().post(`/api/data-sources?branch_id=${branchId}`))
            .send({ name: 'sbf-ds', kind: 'restapi', options: dsOptions('http://sbf-ds.example.com'), scope: 'global' })
            .expect(201)
        ).body.id;
    ...
    describe("hydration marks an app's connected data source is_synced=true", () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-hydrate-conn@tooljet.io',
          firstName: 'git',
          lastName: 'hydrateconn',
        });
        hyOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-hydrate-conn@tooljet.io');
    ...
    describe('hydration cascade marks a referenced module is_synced=true', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-mod-cascade@tooljet.io',
          firstName: 'git',
          lastName: 'modcascade',
        });
        mcOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-mod-cascade@tooljet.io');
      ...
      it('opening a host app cascade-hydrates its referenced module and marks the module is_synced=true', async () => {
        ...
        // module co_relation_id — the value the ModuleViewer must carry.
        ...
        // the referenced module was materialized by the cascade — a direct open is now a no-op.
        ...
        // sanity: the host app itself is synced too
    ...
    describe('app open never pulls from git', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-open-no-clone@tooljet.io',
          firstName: 'git',
          lastName: 'opennoclone',
        });
        oncOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-open-no-clone@tooljet.io');
    ...
    describe('pull skip — token storage + whole-pull skip (git tree SHAs)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-pull-skip@tooljet.io',
          firstName: 'git',
          lastName: 'pullskip',
        });
        psOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-pull-skip@tooljet.io');
    ...
    describe('pull skip — category-level skip leaves that category unreconciled (git tree SHAs)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-pull-skip-cat@tooljet.io',
          firstName: 'git',
          lastName: 'pullskipcat',
        });
        catOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-pull-skip-cat@tooljet.io');
    ...
    describe('push serialization — no DB timestamps in pushed resource files (git tree SHAs)', () => {
      ...
      beforeAll(async () => {
        const { organization } = await createUser(app, {
          email: 'git-push-no-ts@tooljet.io',
          firstName: 'git',
          lastName: 'pushnots',
        });
        tsOrgId = organization.id;
        const { tokenCookie } = await login(app, 'git-push-no-ts@tooljet.io');
      ...
      it('omits created_at / updated_at / git_tree_sha from pushed app version files', async () => {
        ...
        const readVersionFiles = async (branch: string): Promise<{ path: string; text: string }[]> => {
          ...
          const path = await import('path');
          const os = await import('os');
          const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'tj-push-nots-'));
          try {
            const git = simpleGit({
              baseDir: tmpDir,
              timeout: { block: 30000 },
    ...
    // data source → pull → the data source is back. Each `it` uses a FRESH org and resets
    // the shared repo. Against the real Gitea (@group gitsync).
    // ────────────────────────────────────────────────────────────────────────────
    describe('locally-deleted data source is restored on pull (#783)', () => {
      ...
      it('multi-branch: a data source deleted on a feature branch is restored (reactivated) on pull', async () => {
        const ctx = await makeCtx('del-restore-mb@tooljet.io');
      ...
      it('single-branch: a data source deleted on the default branch is restored (re-created) on pull', async () => {
        const ctx = await makeCtx('del-restore-sb@tooljet.io');
