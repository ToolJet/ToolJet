...
describe('TooljetDbDataOperationsService', () => {
  describe('EE (plan: enterprise)', () => {
    ...
    beforeAll(async () => {
      ...
      // that calls createTooljetDbTenantSchemaAndRole), so the tenant role is provisioned here.
      if (tooljetDbAvailable) {
    ...
    describe('environment-scoped-reads', () => {
      describe('plugin context | environment_id', () => {
        afterEach(() => {
          jest.restoreAllMocks();
        });

        it('should carry the resolved environment into context.app.environment_id for a tooljetdb query', async function () {
    ...
    describe('environment-scoped-writes', () => {
      ...
      describe('create_row, update_rows, delete_rows | resolve in the requested environment', () => {
        ...
        it('should have create_row write through the production relation, not development, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should 404 for create_row when this workspace has not promoted the table to production', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should have update_rows write through the production relation, not development, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should 404 for update_rows when this workspace has not promoted the table to production', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should have delete_rows write through the production relation, not development, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should 404 for delete_rows when this workspace has not promoted the table to production', async function () {
          expect(tooljetDbAvailable).toBe(true);
      ...
      describe('bulk_update_with_primary_key & bulk_upsert_with_primary_key | resolve in the requested environment', () => {
        ...
        it('should have bulk_upsert_with_primary_key land rows in production, not development, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);
          ...
          const prodRows = await tjds.query(
            `SELECT * FROM "${workspaceSchema()}"."${prodRelation.id}" WHERE id = 9001`
          );
        ...
        it("should resolve bulkUpsertRowsWithPrimaryKey's write target and its shape read to the same relation", async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should have bulk_update_with_primary_key update production, not development, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);
          ...
          await tjds.query(
            `INSERT INTO "${workspaceSchema()}"."${prodRelation.id}" (id, name) VALUES (9002, 'Before')`
          );
          ...
          const devRows = await tjds.query(
            `SELECT name FROM "${workspaceSchema()}"."${devRelation.id}" WHERE id = 9002`
          );
          expect(devRows[0].name).toBe('Before');
        });

        it('should fail bulk_update_with_primary_key closed for a table not promoted to production', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should fail bulk_upsert_with_primary_key closed for a table not promoted to production', async function () {
          expect(tooljetDbAvailable).toBe(true);
    ...
    describe('sql-and-joins', () => {
      ...
      describe('sql_execution & join_tables | resolve in the requested environment', () => {
        ...
        it('should have sql_execution read production rows, not development rows, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);

          let organizationId: string | undefined;
          try {
            await withRealTransactions(async () => {
              const workspace = await setUpTjdbWorkspace(app, { prefix: 'tjdb-env' });
        ...
        it('should 404 sql_execution when the table has not been promoted to production', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should have join_tables join production relations, not development relations, when production is named', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should 404 join_tables when one of the joined tables has no relation in production', async function () {
          expect(tooljetDbAvailable).toBe(true);
