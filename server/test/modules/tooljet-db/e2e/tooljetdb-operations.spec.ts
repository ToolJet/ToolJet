  ...
  describe('EE (plan: enterprise)', () => {
    ...
    describe('tables', () => {
      ...
      describe('Admin table DDL operations | create, list, delete tables', () => {
        it('should let admin create a table', async function () {
        ...
        it('should create the relation with an id independent of the logical table id, and build the physical table under it', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should transfer ownership of a newly created table to the workspace tenant role', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should let admin list tables', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should let admin delete a table', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should make the drop soft: the table disappears from view_tables and the resolver, its name is free for reuse, and the internal_tables row survives with deleted_at set', async function () {
          expect(tooljetDbAvailable).toBe(true);
    ...
    describe('columns', () => {
      ...
      describe('Table and column DDL round trip | edit_table, add_column, edit_column, drop_column', () => {
        it('should let admin rename a table, add a column, rename that column, then drop a different column', async function () {
          expect(tooljetDbAvailable).toBe(true);
      ...
      describe('view_table | primary key and unique constraint reporting', () => {
        it('should report is_primary_key and is_unique correctly for a table with both', async function () {
          expect(tooljetDbAvailable).toBe(true);
    ...
    describe('foreign-keys', () => {
      ...
      describe('Foreign key DDL round trip | create_foreign_key, update_foreign_key, delete_foreign_key', () => {
        it('should create, update, then delete a foreign key between two tables', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should have view_table report the referenced table by its logical internal_tables.id, not a relation id echoed back from the response', async function () {
          expect(tooljetDbAvailable).toBe(true);
      ...
      describe('GET .../table/:tableId/dependents', () => {
        it('should count a table_id reference and a join-condition-only reference as distinct apps, include a workflow, exclude an old unreleased version, and dedupe multiple queries on one app', async function () {
          expect(tooljetDbAvailable).toBe(true);
    ...
    describe('bulk-upload', () => {
      ...
      describe('Bulk upload | POST /table/:tableName/bulk-upload', () => {
        it('should insert CSV rows into the table named by the relation id and advance its serial sequence', async function () {
          expect(tooljetDbAvailable).toBe(true);
          ...
          await tjDbManager.query(
            `ALTER TABLE "${tenantSchema}"."${originalRelation.id}" RENAME TO "${newRelationId}"`
          );
    ...
    describe('migration-recording', () => {
      ...
      describe('Migration recording | perform() wiring', () => {
        ...
        it('should record a migration and an applied row for every one of the nine structured ops, with column invariants holding after each', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should record the given migration_name for drop_column, delete_foreign_key and drop_table, and fall back to the default name when omitted', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should have a failed DDL leave no migration and no application row, and surface the original Postgres error', async function () {
          expect(tooljetDbAvailable).toBe(true);
        ...
        it('should adjudicate a pending migration on next access: confirm one whose DDL happened, delete one whose DDL did not', async function () {
          expect(tooljetDbAvailable).toBe(true);
    ...
    describe('access-control', () => {
      ...
      describe('End-user access denial | role-based guard', () => {
        it('should deny an end-user table creation (403)', async () => {
