...
const MIGRATION_NAME = 'BackfillTooltipFormatForNavigation1787658025000';
const BATCH_SIZE = 2000;
const TYPE = 'Navigation';

export class BackfillTooltipFormatForNavigation1787658025000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Navigation components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // Navigation already keeps visibility/disabledState under properties; only the new
      // tooltip/tooltipFormat pair needs backfilling. Only fill when unset so an existing
      // value is preserved.
