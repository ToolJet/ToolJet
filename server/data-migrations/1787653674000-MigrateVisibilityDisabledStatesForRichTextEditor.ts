...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForRichTextEditor1787653674000';
const BATCH_SIZE = 2000;
const TYPE = 'RichTextEditor';

export class MigrateVisibilityDisabledStatesForRichTextEditor1787653674000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no RichTextEditor components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that RichTextEditor defines its own boxShadow style instead of relying on the
      // universal one. Only fill a target when unset so the user's saved value is preserved.
