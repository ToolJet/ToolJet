...
const MIGRATION_NAME = 'BackfillTooltipFormatForHtml1787657748000';
const BATCH_SIZE = 2000;
const TYPE = 'Html';

export class BackfillTooltipFormatForHtml1787657748000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Html components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // Html already keeps visibility/disabledState/boxShadow under the right buckets and
      // never had a plain tooltip to move; only the new tooltip/tooltipFormat pair needs
      // backfilling. Only fill when unset so an existing value is preserved.
