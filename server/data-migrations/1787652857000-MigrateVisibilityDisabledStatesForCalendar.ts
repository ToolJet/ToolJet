...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForCalendar1787652857000';
const BATCH_SIZE = 2000;
const TYPE = 'Calendar';

export class MigrateVisibilityDisabledStatesForCalendar1787652857000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Calendar components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // tooltipFormat to pair with the moved tooltip. Move boxShadow (generalStyles→styles)
      // now that Calendar defines its own boxShadow style instead of relying on the universal
      // one. Calendar has no disabledState. Only fill a target when unset so the user's saved
      // value is preserved. Delete the stale keys.
