...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForSpinner1787654153000';
const BATCH_SIZE = 2000;
const TYPE = 'Spinner';

export class MigrateVisibilityDisabledStatesForSpinner1787654153000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Spinner components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that Spinner defines its own boxShadow style instead of relying on the universal
      // one. Spinner has no disabledState. Only fill a target when unset so the user's saved
