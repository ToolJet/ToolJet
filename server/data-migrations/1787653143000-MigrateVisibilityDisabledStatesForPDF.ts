...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForPDF1787653143000';
const BATCH_SIZE = 2000;
const TYPE = 'PDF';

export class MigrateVisibilityDisabledStatesForPDF1787653143000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no PDF components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that PDF defines its own boxShadow style instead of relying on the universal one.
      // PDF has no disabledState. Only fill a target when unset so the user's saved value is
