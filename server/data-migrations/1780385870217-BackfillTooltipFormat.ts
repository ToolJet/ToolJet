...
const MIGRATION_NAME = 'BackfillTooltipFormat1780385870217';
...
export class BackfillTooltipFormat1780385870217 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    console.log(`${MIGRATION_NAME}: [START] Backfill tooltipFormat | Total: ${total}`);
    ...
    while (true) {
      const rows: { id: string }[] = await queryRunner.query(
        `SELECT id FROM components
         ...
         ORDER BY id ASC
         LIMIT $4`,
        [COMPONENT_TYPES, lastId, 'tooltipFormat', BATCH_SIZE]
      );
    ...
    console.log(`${MIGRATION_NAME}: [SUCCESS] Backfill tooltipFormat finished.`);
