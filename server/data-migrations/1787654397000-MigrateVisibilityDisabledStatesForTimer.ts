...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForTimer1787654397000';
const BATCH_SIZE = 2000;
const TYPE = 'Timer';

export class MigrateVisibilityDisabledStatesForTimer1787654397000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Timer components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that Timer defines its own boxShadow style instead of relying on the universal one.
