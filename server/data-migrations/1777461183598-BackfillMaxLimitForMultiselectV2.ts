...
const MIGRATION_NAME = 'BackfillMaxLimitForMultiselectV21777461183598';
const BATCH_SIZE = 2000;
const COMPONENT_TYPES = ['MultiselectV2'];

const MAX_LIMIT_PATCH = JSON.stringify({
  maxLimit: { value: '' },
});

export class BackfillMaxLimitForMultiselectV21777461183598 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(
      `SELECT COUNT(*) FROM components
       WHERE type = ANY($1)
         AND (properties IS NULL OR NOT (properties::jsonb ? 'maxLimit'))`,
      [COMPONENT_TYPES]
    );
    const total = parseInt(count, 10);
    console.log(`${MIGRATION_NAME}: [START] Backfill maxLimit for MultiselectV2 | Total: ${total}`);
    ...
    console.log(`${MIGRATION_NAME}: [SUCCESS] Backfill maxLimit for MultiselectV2 finished.`);
