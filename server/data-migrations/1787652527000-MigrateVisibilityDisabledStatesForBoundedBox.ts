...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForBoundedBox1787652527000';
const BATCH_SIZE = 2000;
const TYPE = 'BoundedBox';

export class MigrateVisibilityDisabledStatesForBoundedBox1787652527000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no BoundedBox components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // backfill tooltipFormat to pair with the moved tooltip. Move boxShadow (generalStyles→styles)
      // now that BoundedBox defines its own boxShadow style instead of relying on the universal one.
      // Only fill a target when unset so the user's saved value is preserved. Delete the stale keys.
