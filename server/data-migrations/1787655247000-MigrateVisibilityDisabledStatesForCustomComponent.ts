...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForCustomComponent1787655247000';
const BATCH_SIZE = 2000;
const TYPE = 'CustomComponent';

export class MigrateVisibilityDisabledStatesForCustomComponent1787655247000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no CustomComponent components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // Move visibility (styles→properties) and tooltip (general→properties); backfill
      // tooltipFormat. CustomComponent already owns its own boxShadow style, so no boxShadow
      // move is needed. CustomComponent has no disabledState. Only fill a target when unset so
      // the user's saved value is preserved. Delete the stale keys.
      await queryRunner.query(
        `UPDATE components SET
             ...
             styles = (COALESCE(styles::jsonb,'{}'::jsonb) - 'visibility')::json,
