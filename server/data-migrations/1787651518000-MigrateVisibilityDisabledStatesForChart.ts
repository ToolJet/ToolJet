...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForChart1787651518000';
const BATCH_SIZE = 2000;
const TYPE = 'Chart';

export class MigrateVisibilityDisabledStatesForChart1787651518000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Chart components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that Chart defines its own boxShadow style instead of relying on the universal one.
