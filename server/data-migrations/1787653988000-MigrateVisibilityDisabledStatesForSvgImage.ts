...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForSvgImage1787653988000';
const BATCH_SIZE = 2000;
const TYPE = 'SvgImage';

export class MigrateVisibilityDisabledStatesForSvgImage1787653988000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no SvgImage components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that SvgImage defines its own boxShadow style instead of relying on the universal
      // one. SvgImage has no disabledState. Only fill a target when unset so the user's saved
