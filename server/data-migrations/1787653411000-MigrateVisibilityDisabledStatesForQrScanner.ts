...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForQrScanner1787653411000';
const BATCH_SIZE = 2000;
const TYPE = 'QrScanner';

export class MigrateVisibilityDisabledStatesForQrScanner1787653411000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no QrScanner components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that QrScanner defines its own boxShadow style instead of relying on the universal one.
