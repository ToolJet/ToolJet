...
const MIGRATION_NAME = 'MigrateVisibilityDisabledStatesForMap1787649608000';
const BATCH_SIZE = 2000;
const TYPE = 'Map';

export class MigrateVisibilityDisabledStatesForMap1787649608000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Map components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // now that Map defines its own boxShadow style instead of relying on the universal one.
