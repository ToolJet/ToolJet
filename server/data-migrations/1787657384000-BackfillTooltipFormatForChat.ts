...
const MIGRATION_NAME = 'BackfillTooltipFormatForChat1787657384000';
const BATCH_SIZE = 2000;
const TYPE = 'Chat';

export class BackfillTooltipFormatForChat1787657384000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Chat components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // Chat already keeps visibility/disableInput/boxShadow under the right buckets; only
      // tooltip (general→properties) and the new tooltipFormat pairing need backfilling. Only
      // fill a target when unset so the user's saved value is preserved. Delete the stale key.
