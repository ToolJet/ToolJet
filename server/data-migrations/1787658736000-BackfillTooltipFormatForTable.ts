...
const MIGRATION_NAME = 'BackfillTooltipFormatForTable1787658736000';
const BATCH_SIZE = 2000;
const TYPE = 'Table';

export class BackfillTooltipFormatForTable1787658736000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) FROM components WHERE type = $1`, [TYPE]);
    const total = parseInt(count, 10);
    if (total === 0) {
      console.log(`${MIGRATION_NAME}: no Table components found.`);
      return;
    }
    ...
    while (true) {
      ...
      // Table already keeps visibility/disabledState correctly under properties; only the new
