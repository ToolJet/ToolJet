...
export interface IAppsUtilService {
  ...
  all(user: User, page: number, searchKey: string, type: string, isGetAll: boolean, showAll?: boolean): Promise<AppBase[]>;
  count(user: User, searchKey: string, type: string, showAll?: boolean): Promise<number>;
