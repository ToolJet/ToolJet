...
- **App metadata on versions** — for non-workflow apps, `appName`/`slug`/`icon`/`isPublic` live on `app_versions` rows (branch-aware); workflows keep `slug` on `apps` (see `subscribers/apps.subscriber.ts`). `workflowEnabled` is the fifth version-level column and applies to workflows only.
...
- `overlayAppMetadata` reads five version-level columns, not four — `appName`/`slug`/`icon`/`isPublic`/`workflowEnabled` — so callers keep seeing one flat field per app. The repository picks the branch arm conditionally (`useBranchArm`); adding a sixth column means touching the `COALESCE` set, the overlay, **and** the import whitelist in `app-import-export.service.ts`, which enumerates fields rather than spreading them.
...
- **App open never pulls from git.** EE `getOne` hydrates only when the branch has an `is_stub` version; a materialized (non-stub) draft is served straight from the DB, and `hydrateStaleReferencedModules` likewise skips modules that are already materialized. An earlier version re-cloned the repo on every open to compare git's tree SHA against the draft's stored `git_tree_sha` — one clone per app open (plus one per referenced module). Keeping an outdated draft fresh is the pull path's job. Known gap: workspace pull marks a changed non-stub draft `outdated` (leaves `git_tree_sha` stale, see `ee/platform-git-sync/pull.service.ts` `classifyEntry`) but nothing acts on that marker at open time, so such a draft keeps serving its last materialized content until it is re-stubbed or re-imported. Don't reintroduce a git call on the open path to close it — give the pull a cheap DB staleness signal instead.
