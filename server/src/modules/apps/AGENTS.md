...
- App name/slug uniqueness is enforced at the DB level by partial unique indexes on `app_versions` (`app_versions_app_name_branch_id_unique`, `app_versions_slug_branch_id_unique`) plus two triggers: `enforce_app_versions_app_name_branch_unique` (name) and `trg_app_versions_default_branch_slug_unique` (slug, instance-wide across default-branch rows). **Slug uniqueness is scoped per `apps.type`** — a front-end app and a module (or workflow) may share a slug on purpose, they're separate product surfaces with separate dashboards. `util.service.ts#update` mirrors this with its own pre-flight checks (both the same-branch and the instance-wide default-branch query filter on `a.type = app.type`) so a collision surfaces as a friendly 400 before the trigger fires — don't add/copy a slug check here without the type filter, or it'll reject an app/module sharing a slug that the DB would otherwise allow.
- **Gotcha:** slug *resolution* (`AppsRepository.findBySlug`, `findAppBySlug`, `retrieveAppDataUsingSlug`) does NOT filter by `apps.type` — it matches on `app_versions.slug` alone (tie-broken by `is_synced`/`updated_at`). `findBySlug` backs `ValidAppGuard`, the route guard behind the App Builder's `/apps/:id-or-slug` editor URL. So while writes correctly keep app/module slugs as separate namespaces, a read by slug is not type-aware: if an app and a module ever end up sharing a slug in the same workspace (racy insert, data migration, git pull, etc.), which one the builder opens is decided by the tie-break order, not by the caller's intended type. No known live bug from this today, but a change here needs a type filter added to the resolvers, not just the write-side checks.
