  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await tooljetDbManager.transaction(async (transactionalEntityManager) => {
        ...
        await transactionalEntityManager.queryRunner.query('SELECT pg_advisory_lock(123456788)');
      ...
      `);
          ...
          await transactionalEntityManager.queryRunner.query('SELECT pg_advisory_unlock(123456788)');
        }
      });
...
export async function reconfigurePostgrestWithoutSchemaSync(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await tooljetDbManager.transaction(async (transactionalEntityManager) => {
        // Use advisory locks to prevent concurrent execution
        await transactionalEntityManager.queryRunner.query('SELECT pg_advisory_lock(123456789)');
        ...
        try {
          ...
          // Always release the advisory lock
          await transactionalEntityManager.queryRunner.query('SELECT pg_advisory_unlock(123456789)');
        }
      });
