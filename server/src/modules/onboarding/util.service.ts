...
export class OnboardingUtilService implements IOnboardingUtilService {
  protected readonly logger = new Logger(OnboardingUtilService.name);
    ...
    return await dbTransactionWrap(async (manager: EntityManager) => {
      ...
      if (signingUpOrganization) {
        ...
        if (!isCloudEdition && response) {
          ...
          if (!userOrg) this.logger.warn('WARNING: userOrg not found for defaultOrganizationId:', user.defaultOrganizationId);
