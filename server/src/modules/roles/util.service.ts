...
export class RolesUtilService implements IRolesUtilService {
  ...
  async checkIfBuilderLevelResourcesPermissions(
    ...
    return await dbTransactionWrap(async (manager: EntityManager) => {
      ...
      // Restrict-only data source perms (no Configure/Build with) are end-user safe
      const isBuilderLevelDataSourcePermissions = allPermission
        .filter((permissions) => permissions.type === ResourceType.DATA_SOURCE)
        .some((permissions) => {
          const dsPermission = permissions.dataSourcesGroupPermission;
          return dsPermission?.canConfigure === true || dsPermission?.canUse === true;
        });
