...
import { UpdateUserDto, WorkspaceDto, UpdateGivenWorkspaceDto, CreateUserDto, UpdateUserMetadataDto, GetWorkspaceUsersByGroupsDto } from './dto';
...
export class ExternalApisController implements IExternalApisController {
  ...
  async getWorkspaceUsersByGroups(
    @Param('workspaceId') workspaceId: string,
    @Body() body: GetWorkspaceUsersByGroupsDto
  ) {
    throw new NotFoundException();
  }
}
