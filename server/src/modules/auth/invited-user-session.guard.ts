...
export class InvitedUserSessionAuthGuard extends AuthGuard('jwt') {
	  ...
	  async canActivate(context: ExecutionContext): Promise<any> {
		...
		if (!workspaceInviteToken) {
			throw new BadRequestException('The workspace invite token is missing from the request parameters. Please provide a valid token to proceed.');
		}
		...
		try {
			...
			throw new BadRequestException('Invalid or expired invitation session. Please check your invitation and try again.');		
		}
		...
		if(isValidUser){
			...
			if(request.user?.email !== invitedUserEmail){
				const errorResponse = {
					message: { error: 'Your session does not match the invited user. Please logout and login to the invited user email account.', isInvitationTokenAndUserMismatch:true },
				};
