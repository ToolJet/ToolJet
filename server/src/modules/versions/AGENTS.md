...
- `checkTablesPromotableToEnvironment(appId, versionId, targetEnvironmentId, targetEnvironmentName, sourceEnvironmentId, organizationId, manager)` still hard-blocks (throws `BadRequestException`) a table with zero relation in the target environment, unchanged. `versionId` scopes the underlying `findTooljetDbTables` lookup to that one version — without it, a table referenced only by an abandoned draft (never promoted, never revisited) blocked every future promote of the app with no UI showing why. It additionally returns `Promise<{ warnings: TableBehindWarning[] }>` (not `void`) for tables that DO resolve a target relation but are behind on migrations — that half is warn-only, never blocks. Both promote call sites — interactive `server/ee/versions/service.ts` and External API `autoDeployApp` (`server/ee/external-apis/service.ts`) — thread `warnings` through as a `tableWarnings` field on their response. Treat `tableWarnings` as part of the promote response's wire format now.
