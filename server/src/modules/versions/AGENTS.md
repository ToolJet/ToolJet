...
- `checkTablesPromotableToEnvironment(appId, targetEnvironmentId, targetEnvironmentName, sourceEnvironmentId, organizationId, manager)` is warn-only, never blocks: returns `Promise<{ warnings: TableBehindWarning[] }>`, not `void`. Both promote call sites — interactive `server/ee/versions/service.ts` and External API `autoDeployApp` (`server/ee/external-apis/service.ts`) — thread its `warnings` through as a `tableWarnings` field on their response. Treat `tableWarnings` as part of the promote response's wire format now.
