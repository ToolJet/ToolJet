...
import { UserCountGuard } from '@licensing/guards/user.guard';
import { EditorUserCountGuard } from '@licensing/guards/editorUser.guard';
import { AllowPersonalWorkspaceGuard } from '@instance_settings/personal-workspace.guard';
...
import { SwitchWorkspaceAuthGuard } from '@module/auth/switch-workspace.guard';
...
export class AppController {
  ...
  async superAdminLogin(@Body() appAuthDto: AppAuthenticationDto, @Res({ passthrough: true }) response: Response) {
    const user = await this.jwtService.findByEmail(appAuthDto.email);
  ...
  async getSessionDetails(@User() user, @Query('appId') appId: string, @Query('workspaceSlug') workspaceSlug: string) {
    return await this.authService.getSessionDetails(user, appId, workspaceSlug);
  }
