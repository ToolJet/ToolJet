...
export class FoldersController {
  ...
  async create(@Request() req, @Body() createFolderDto: CreateFolderDto) {
    ...
    const folder = await this.foldersService.create(req.user, createFolderDto.name, createFolderDto.type);
  ...
  async update(@User() user, @Param('id') id, @Body() updateFolderDto: UpdateFolderDto) {
    ...
    const folder = await this.foldersService.update(id, updateFolderDto.name);
    return decamelizeKeys(folder);
  }
