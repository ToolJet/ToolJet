...
- _Visual app builder:_ 35+ built-in responsive widgets such as Tables, Charts, Lists, Forms, Progress bars, and more.
- _20+ data sources:_ connect to databases, cloud storage, and APIs.
- _Desktop & mobile:_ layout widths can be customized to support different screens.
- _Self-host:_ (supports Docker, Kubernetes, Heroku, AWS EC2, Google Cloud Run, and more).
- _Collaborate:_ add comments anywhere on the canvas and tag your team members.
- _Extend with plugins:_ use our [commandline tool](https://www.npmjs.com/package/@tooljet/cli) to easily bootstrap new connectors.
- _Version control:_ every application has different versions with a proper release cycle.
- _Run JS code:_ ability custom JavaScript snippets
- _Granular access control_ on organization-level and app-level.
- _Low-code:_ write JS code almost anywhere in the builder. For example, the color property of text can be set to `status === 'success' ? 'green' : 'red'`
- _No-code query editors:_ for all supported data sources.
- _Join and transform data:_ transform query results using just JavaScript code.
- _Secure:_ All the credentials are securely encrypted using `aes-256-gcm`.
- _Doesn't store data:_ ToolJet acts only as a proxy and doesn't store any data.
- _SSO:_ Supports multiple SSO providers
...
Kindly read our [Contributing Guide](CONTRIBUTING.md) to learn and understand our development process, how to propose bug fixes and improvements, and how to build and test your changes to ToolJet. <br>
