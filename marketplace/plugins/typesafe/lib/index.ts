...
export default class TypeSafe implements QueryService {
  ...
  private parseState(raw: QueryOptions['state']): EvaluateRequest['state'] {
    if (raw === undefined || raw === null || raw === '') {
      throw new QueryError(
        ...
        'State is required: the text or record to decide about.',
        {}
      );
    }
  ...
  private parseQuestions(raw: QueryOptions['questions']): Record<string, Question> {
    if (raw === undefined || raw === null || raw === '') {
      throw new QueryError(
        'Query could not be completed',
        'Questions are required: a JSON map of question id to a typed question.',
        {}
      );
    }
    ...
    for (const id of ids) {
      ...
      if (!q || typeof q !== 'object') {
        throw new QueryError(
          'Query could not be completed',
          `Question "${id}" must be an object with type and instructions.`,
          {}
        );
      }
      ...
      if (q.type === 'score') {
        if (!Array.isArray(q.criteria) || q.criteria.length < 2) {
          throw new QueryError(
            'Query could not be completed',
            `Question "${id}": a score needs criteria as an ordered list of at least two levels.`,
            {}
          );
        }
      }
  ...
  private apiKey(sourceOptions: SourceOptions): string {
    const key =
      this.authType(sourceOptions) === 'openrouter' ? sourceOptions.openrouter_api_key : sourceOptions.api_key;
    if (!key || !key.trim()) {
      throw new QueryError(
        'Query could not be completed',
        this.authType(sourceOptions) === 'openrouter'
