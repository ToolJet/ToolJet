...
import { SourceOptions, QueryOptions, ConvertedFormat, AccessDetailsFromParams } from './types';
...
export default class Gmail implements QueryService {
  private validateSourceOptions(sourceOptions: SourceOptions) {
    ...
    if (!client_id?.value || !client_secret?.value) {
      const error = new Error('Missing required source options: client_id and client_secret.');
  ...
  authUrl(sourceOptions: SourceOptions): string {
    ...
    const clientId = oauth_type?.value === 'tooljet_app' ? process.env.GOOGLE_CLIENT_ID : client_id?.value;
  ...
  async refreshToken(sourceOptions: any) {
    if (!sourceOptions['refresh_token']) {
      throw new QueryError('Query could not be completed', 'Refresh token not found in source options', {
    ...
    try {
      ...
      if (!(response.statusCode >= 200 && response.statusCode < 300)) {
        throw new QueryError('Query could not be completed', 'Could not connect to Gmail', {
      ...
      if (result['access_token']) {
        ...
        throw new QueryError('Query could not be completed', 'Access token not found in the response', {
          statusCode: response.statusCode,
          statusText: response.statusMessage,
          error: response.body,
        });
      ...
      throw new QueryError('Query could not be completed', error.message || 'An unexpected error occurred', {
