...
import { BedrockRuntimeClient, BedrockRuntimeServiceException } from '@aws-sdk/client-bedrock-runtime';
...
export default class AWSBedrock implements QueryService {
  async run(sourceOptions: SourceOptions, queryOptions: QueryOptions, dataSourceId: string): Promise<QueryResult> {
    try {
      ...
      switch (queryOptions.operation) {
        case 'generate_content': {
          if (!queryOptions.model_id || !queryOptions.request_body) {
            throw new QueryError('Validation failed', 'Model ID and request body are required', {
              validation: {
        ...
        case 'list_foundation_models': {
          ...
          const params: Record<string, any> = {
            ...(queryOptions.customization_type?.length ? { customization_type: queryOptions.customization_type } : {}),
            ...(queryOptions.inference_type?.length ? { inference_type: queryOptions.inference_type } : {}),
            ...(queryOptions.output_modality?.length ? { output_modality: queryOptions.output_modality } : {}),
            ...(queryOptions.provider?.length ? { provider: queryOptions.provider } : {}),
          };
      ...
      if (error instanceof BedrockRuntimeServiceException || error instanceof BedrockServiceException) {
        throw new QueryError('AWS Bedrock Error', error.message, {
          aws_error: {
            name: error.name,
      ...
      throw new QueryError('Operation failed', error.message || 'Unknown error occurred', {
  ...
  private validateCredentials(sourceOptions: SourceOptions): void {
    ...
    if (!hasAccessKey) {
      throw new QueryError('Invalid credentials configuration', 'Access key and secret access key are required', {
        validation: {
